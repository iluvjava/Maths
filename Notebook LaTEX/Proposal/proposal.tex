...
\section{Catalyst accelerations}\label{sec:catalyst}
    ...
    \subsection{The second Catalyst Acceleration paper}\label{ssec:catalyst-second-paper}
        Lin et al.'s second paper on Catalyst Acceleration \cite{lin_catalyst_2018} describes the new option of relative termination criteria (which we stated here in Definition \ref{def:catalyst-termination-c2}) for $\mathbb M$ to evaluate the inexact solutions of $\mathcal J_{\kappa^{-1}F}^{\epsilon_k} y_k$. 
        This condition allows the the use of relative error sequence $\delta_k$ which has advantages that: it gives easier convergence anlaysis, it doesn't need knowledge of $F^*$ and it can be generalize to the nonconvex case. 
        \par
        This section summarizes major results in Lin et al's second Catalyst paper \cite{lin_catalyst_2018}. 
        ...
        \subsubsection{Consequences of the inner loop termination criteria}
            Lemma \ref{lemma:sufficient-c1} describes a sufficient conditions to verify the membership of $\bar z \in \mathcal J_{\kappa^{-1}F}^\epsilon (x)$ through the proximal gradient operator on $F:= f_\kappa(z) + g(z)$ which has practical importance because it translate criterion C1 into a implementable criteria in the algorithm with the proximal gradient operator. 
            It contribute to theoretical analysis because it bounds on the true error on the gradient of Moreau Envelope at the point $x$. 
            \par 
            ...
            \begin{definition}[Catalyst Acceleration all in one]\label{def:catalyst-all-in-one}
                ...
                \end{tcolorbox}
            \end{definition}
            \begin{assumption}[Warm start conditions for relative error C2]
            \;\label{prop:warm-start-with-c2}\\ 
            Assume 
            ...
            \end{remark}
            \begin{theorem}{Inner loop complexity with relative error C2}
