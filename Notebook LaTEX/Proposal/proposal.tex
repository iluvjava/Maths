...
\section{Catalyst accelerations}\label{sec:catalyst}
    ...
    \subsection{Potential future research}\label{ssec:catalyst-future}
        ...
        \subsubsection{Necoara et al.'s comments on Catalyst Acceleration}
            ...
            Paquette et al. \cite{paquette_catalyst_2018} were inspired by \cite{ghadimi_accelerated_2016} and showed that the Catalyst algorithm with a different Termination criterion C2 that is based on the gradient (see equation (13), (14) in their paper) can adapt to nonconvex settings to convergence on the norm of the gradient. 
            However, it's at the expense of one additional evaluation of the proximal point method, and it requires knowledge of the function value for the outer loop. 
            ...
            \par
            Attaining stronger convergence results on iterates, convergence to stationarity and function value for Nesterov's accelerated gradient methods in general requires intense amount of works. 
            In fact, these type of convergences remains nontrivial in the convex settings in general. 
            See Bauchke et al. for example, when the minimizer of $F$ doesn't exist. 
            To limit the scope, we focus on Nesterov's acceleration which differs from inertial momentum from Polyak.
            The relationship between Polyak and Nesterov momentum acceleration remains a non-trivial topics. 
            In fact, both type of acceleration can present at the same time, see 
            \par
            ...
            \begin{enumerate}
                ...
                \item Nesterov's accelerations and with inexact gradient oracles. 
            \end{enumerate}
            ...
            \todoinline{
                ...
                \begin{enumerate}
                    ...
                    \item ``Study of the behavior of Nesterov Accelerated Gradient in a nonconvex setting: the strongly quasar convex case."
        ...
        \subsubsection{Our ideas on future works of Catalyst Acceleration}
            ...
            \end{quote}
            This is a detail in the Catalyst framework, and it may not be obvious to the readers that the complexity assumption is sufficient to accommodates objective functions $F$ to be the class of all $L$-Lipschitz smooth function. 
            ...
            \begin{enumerate}
                ...
                \item show that removing the assumption will yield non-trivial answers for the complexity analysis of Catalyst as a whole by existing theorems in the Nesterov's lower bound complexity results for black box smooth/nonsmooth objectives;
                \item show that some other the existing literatures that seems to hold the answer but actually doesn't address the question; 
                \item discuss potential relevant literatures to addressing the question. 
            \end{enumerate}
            \par
            ...
            \par
            From a complexity theory point of view, Catalyst acceleration didn't attempt at narrowing the lower complexity bound for functions that are convex and nonsmooth and lack of any exact proximal oracle. 
            ...
            \begin{quotation}
                Suppose that the objective function $F$ is no $L$-Lipschitz continuous hence Assumption \ref{ass:linear-convergence-inner-loop} is false. 
                If we assume that $F$ is any convex function (potentially nonsmooth and without any smooth parts), would there exist any kind of regularization parameters $(\kappa_k)_{k \ge 0}$, error sequence $\epsilon_k$ such that the overall complexity of Catalyst retains convergence rate equals or faster than $\mathcal O\left(1/\sqrt{k}\right)$ (The lower bound of all first order blackbox convex optimization)? 
            \end{quotation}
            ...
            \par
            To the best of our knowledge, nobody has yet answered the question directly in the literature, but we found some relevant literatures after some preliminary research. 
            Nesterov 2014 \cite{nesterov_universal_2015}, he considers the method of accelerated gradient for function with Holder continuous gradient.
            Nesterov continued and expanded on the results obtained by Devolder et al. 2014 \cite{devolder_first-order_2014} on inexact oracles for blackbox optimizations which showed that the inexact oracle for Lipschitz smooth function can be viewed as exact for another perturbed function with Holder continuous gradient. 
            He proposed three first order algorithms where the last one, a variant of accelerated gradient method that converges optimally for all functions with Holder continuous gradient without knowing the Holder continuity constant in advance. 
            ...
            \par
            Of course, the class of holder continuous gradient doesn't encapsulate the class of all convex function where inexact proximal point can be applied. 


            \todoinline{Add research plans and literature reviews here. }

        
        \subsubsection{Applications for Catalyst Accelerations and its variants}
            \todoinline{
                Considers some specific applications of the Catalyst Acceleration method in the literatures. 
            }
