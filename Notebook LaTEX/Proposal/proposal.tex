...
\section{Introduction}
    ...
    \par
    In this proposal we explore the Goldilocks zones between these topics: Nesterov's acceleration, algorithms with inexact evaluation of the proximal point operator, and applications.  
    We want to identify the ``chemistry'' between various properties of functions and the affect it has on the designs/behaviors of first order algorithm for continuous optimization. 
    \par
    \textbf{Organizations now follows.} 
    Section \ref{sec:unify-nes-acceleration}, proposes the method of ``Relaxed Weak Accelerated Proximal Gradient (R-WAPG)'' as the foundation to describe several Euclidean variants of Accelerated Proximal Gradient (APG) method in the literatures. 
    The convergence analysis of R-WAPG gives convergence of APG the momentum is much weaker compare to the Nesterov's update rule. 
    R-WAPG can is generic and can describe well-established variants of FISTA in the literatures with alternative choices of step sizes and beyond. 
    \par
    ...
    Numerical experiments are presented which shows the competitiveness of our algorithm. 
    Potential future direction of research is given by the end of the section for the R-WAPG framework. 
    \par
...
\section{Preliminaries}
    ...
    \subsection{Fundamentals in convex analysis}
        ...
        So it has 
        \todo{
            $\square$ Add Rockafellar's red book \\ $\square$ Cite Section 10 (pg 82) of that book
        }
        \hl{$\reli(\dom F)\subseteq \dom(\partial F) \subseteq \dom F$.} 
        \par
...
\section{Unifying NAG, and weakening the sequence assumption for convergences}\label{sec:unify-nes-acceleration}
    This section gives results of the theoretical part  of our draft paper. 
    It will introduce major results and claims achieved during our research in each of the subsections. 
    All theorems and claims stated in this section are proved in the draft paper. 
    We will start by introducing the context of our research. 
    \par
    Assume we want to solve the convex optimization problem: $F^* = \min_{x \in \RR^n} \{F(x)\}$ and assume $F: \RR^n \rightarrow \RR$ is $L$ Lipschitz smooth for the simplicity of discussion. 
    One appealing candidate method for solving it is the Nesterov's accelerated gradient methods (NAG) proposed back in 1983 \cite{nesterov_method_1983}. 
    It's well known that if minimizer $x^*$ exists for $F$, the method achieves a convergence rate of $\mathcal O(1/k^2)$ for $F(x_k) - F^*$.     
    It is considered optimal for all class of $L$ Lipschitz smooth convex function and see Chapter 2 of Nesterov \cite{nesterov_lectures_2018} for information. 
    The guaranteed convergence rate is better than $\mathcal O(1/k)$ exhibited by gradient descent. 
    \par
    ...
    \end{align}
    Unfortunately, this algorithm sped up the convergence rate for the class of all convex function, it becomes slower for the subset of $\mu > 0$ strongly convex function. 
    ...
    \par
    There are other alternatives to restarting if it's known in advance that $F$ would be $\mu > 0$ strongly convex. 
    Let $F:\RR^n \rightarrow \RR$ be a $L$ Lipschitz smooth and $\mu > 0$ function. 
    In Walkington's writing \cite{noel_nesterovs_nodate}, 
    he showed that there exists a variant of NAG that achieves linear convergence rate of $\mathcal O((1 - \sqrt{\kappa})^k)$ where $\kappa = \mu/L$. 
    This convergence rate is strictly better than $\mathcal O((1 - \mu/L)^k)$ from gradient descent. 
    However, the variant has a fixed momentum parameter $\theta_{k + 1} = (\sqrt{\kappa} - 1)(\sqrt{\kappa} + 1)^{-1}$ in Equation \ref{eqn:nag_example} and has convergence claim for F with $\mu > 0$. 
    ...
    \par
    Some other researches in the literatures focus on the convergence of the iterates, and it has much to do with the momentum sequence $(\theta_k)_{k\ge 0}$ displayed in Equation \ref{eqn:nag_example}. 
    ...
    \par
    The content of our draft paper answers the question: ``Is it possible to describe something about the NAG algorithm for a set of sequence that is non-traditional?''. 

    \textbf{A summary of our contributions now follow.}
    Our contributions are theoretical and practical. 
    Out results are based the assumption $F = f + g$ where $g:\RR^n \rightarrow \overline\RR$ is convex, and $f$ is an $L$-Lipschitz smooth and $\mu \ge 0$ strongly convex. 
    We relax the traditional choice of the sequence $\theta_k$ in Equation \ref{eqn:nag_example} and showed an upper bound of the optimal gap for the sequences $(\alpha_k)_{k \ge0}, (\rho_k)_{k \ge 0}$ that satisfy
    \begin{align*}
        \alpha_0 &\in (0, 1], 
        \\
    ...
    \end{align*}
    Our first main result shows that if $\theta_{k + 1} = (\rho_k\alpha_k(1 - \alpha_k)/(\rho_k\alpha_k^2 + \alpha_{k + 1}))$, using the generic algorithm R-WAPG we proposed in Definition \ref{def:wapg} with Proposition \ref{prop:wagp-convergence}, \ref{prop:r-wapg-momentum-repr}, it can show that the gap $F(x_k) - F(x^*)$ is bounded by:
    \begin{align*}
        \mathcal O\left(
            \left(
        ...
        \right). 
    \end{align*}
    Our second main result shows that, assuming $\mu = 0$ then there exists the choice of $(\alpha_k)_{k \ge 0}$ $\rho_k > 1$ such that our R-WAPG reduces to the variant of FISTA proposed by Chambolle, Dossal \cite{chambolle_convergence_2015}, and we are able to show the same convergence rate in Theorem \ref{thm:r-wapg-on-cham-doss}. 
    In the special case where $(\alpha_k)_{k \ge 0}$ has $\rho_k = 1 \;\forall k \ge 0$, and assuming $\mu = 0$, R-WAPG reduces to the FISTA algorithm proposed in Beck \cite{beck_fast_2009}.
    If $\mu > 0, \rho_k = 1\;\forall k \ge 0$, it reduces to the V-FISTA by Beck \cite{beck_first-order_2017}. 
    In Theorem \ref{thm:fixed-momentum-fista}, we demonstrated that R-WAPG frameworks gives a linear convergence claim for all APG where $\alpha_k := \alpha \in (\mu/L, 1)$ is a fixed constant parameter and $F$ is $\mu > 0$ strongly convex. 
    \par
    ...
    It doesn't require $L, \mu$ in prior, so it determines the value of $\theta_t$ by estimating $D_f$ on iterates $y_{k}, y_{k + 1}$ with minimal computational cost. 
    We conducted ample amount of numerical experiments to show that it has a favorable convergence rate in practice and exposes its similarity in terms of behaviors to the FISTA with monotone restart under certain situations. 
    \par
    \textbf{Notations, and assumptions now follows. }
    ...
    \par
    \textbf{Organization now follows. }
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    \subsection{Future works for R-WAPG}
        The R-WAPG framework neglected one intriguing detail buried in the literatures. 
...
\section{Catalyst accelerations and future works}\label{sec:catalyst}
    ...
    \begin{assumption}\label{ass:catalyst1}
        ...
        We denote $\mathcal J_{\beta^{-1}F}$ to be the resolvent operator for subgradient of $F$. 
    \end{assumption}
