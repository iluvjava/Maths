...
\section{Catalyst accelerations and future works}\label{sec:catalyst}
    ...
    \subsection{Introduction to Catalyst}
        ...
        This algorithm is often referred to as the inner loop. 
        The outer loop controls where the proximal point $y_{k}$ is evaluated at and computes Nesterov's momentum extrapolations. 
        \par
        The error sequence $(\epsilon_k)_{k \ge 0}$ depends on the properties of the objective function. 
        For example, when $F$ has $\mu = 0$, 
        \todo{
            Lin's paper, Theorem 3.3
        }
        {\hl{Lin }}\cite[Theorem 3.3]{lin_universal_2015} suggested $\alpha_0 = (\sqrt{5} - 1)/2$, and 
        \begin{align}
        ...
        This makes the iteration complexity of just the outer loop optimal. 
        In addition, to achieve optimal convergence for the outer loop when $\mu > 0$ for $F$, 
        \todo{
        ...
        Take note that, the error sequence has $F^*$ the optimal value in both cases. 
        ...
        \par 
        The following parts will complement the report and describe ideas on for choosing the termination criteria of the inner loop to evaluate $\mathcal J_{\kappa^{-1}F}^{\epsilon_k} y_k$ which is the focus of his second paper on Catalyst Acceleration \cite{lin_catalyst_2018}. 
        To elucidate the matters consider the model function $\mathcal M_F^{\kappa^{-1}}(x; y)$ is $\mu + \kappa$ strongly convex, and therefore it admits the Polyak error bound condition: 
        \begin{align*}
            (\forall x\in \RR^n) \quad 
            ...
            &\le (\kappa + \mu)\dist
                ...
                \right). 
        \end{align*}
        By choosing $x$ such that $\dist(\mathbf 0, \partial \mathcal M_F^{1/\kappa}(x; y)) \le \epsilon$, it ensures $x \in \mathcal J^\epsilon_{\kappa^{-1}F}(y)$. 
        ...
        \begin{enumerate}
            \item A relative termination criteria that bounds the gradient of the problem in the inner loop, which results in a error sequence without $F^*$, and simplier proof without using the Nesterov's estimating sequence. 
            \item A more comprehensive convergence proof, for 2 different type of error bounds with, including the smooth plus nonsmooth compositie objective. 
        \end{enumerate}
