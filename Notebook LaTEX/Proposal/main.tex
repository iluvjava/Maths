...
\section{Catalyst accelerations and future works}\label{sec:catalyst}
    ...
    \subsection{Introduction to Catalyst}
        ...
        \todo{
            $\blacksquare$ Add old Dima's paper on incremental methods here. Paper: ``Incremental gradient, subgradient, and proximal methods for convex optimization: A survey''. 
            \\
            $\square$ Cite them. 
        }
        ...
        Typically, an iterative scheme (i.e: VRM) with known complexity is assigned to find inexact proximal iterates $x_k \in {\mathcal J}_{\kappa^{-1}F}^{\epsilon_k} y_{k - 1}$. 
        ...
        \todoinline{\noindent
            \begin{enumerate}
                \item State outer loop iteration complexity given error sequence for 
                \begin{enumerate}
                    \item Strongly convex objective,
                    \item Non-strongly but strongly convex objective. 
                \end{enumerate}
                \item State the linear convergence rate assumption of the inner function. 
                \item State the iteration complexity of the inner loop given the specific error sequence. 
        ...
        \subsubsection{Outer loop iteration complexity}
            ...
            The theorem that follows state the error sequence required for the outer loop to retain optimal convergence rate, they are Theorem 3.3, 3.1 in Lin et al. \cite{lin_universal_2015}. 
            \begin{theorem}[Outer loop convergence strongly convex]
                
            \end{theorem}
            \begin{theorem}[Outer loop convergence convex but not strongly convex]
                
            \end{theorem}
            For example, when $F$ has $\mu = 0$, 
            \todo{
                Lin's paper, Theorem 3.3
            }
            {\hl{Lin }}\cite[Theorem 3.3]{lin_universal_2015} suggested $\alpha_0 = \left(\sqrt{5} - 1\right)/2$, and 
            \begin{align}
                \epsilon_k = \frac{2(F(x_0) - F^*)}{9(k + 2)^{4 + \eta}}, 
            \end{align}\label{eqn:catalyst-error-seq-cnvx}
            
            where $\eta > 0$ is any number, i.e: $\eta = 0.1$. 
            Then, the convergence of $F(x_k) - F^*$ with $(x_k)_{k \ge 0}$ produced by the outer loop is $\mathcal O(1/k^2)$. 
            In this scenario, the iteration complexity of the outer loop optimal. 
            To achieve optimal convergence for the outer loop when $\mu > 0$ for $F$, 
            \todo{
                Lin's first Catalyst, Theorem 3.1. 
            }
            {\hl{Lin }}\cite[Theorem 3.1]{lin_universal_2015}
            suggested $\alpha_0 = \sqrt{\mu/(\kappa + \mu)}$ and the error sequence: 
            \begin{align}
                \epsilon_k = \frac{2}{9}(F(x_0) - F^*)(1 - \rho)^k \quad \text{with } \quad 
                \rho < \sqrt{\frac{\mu}{\mu + \kappa}}. 
            \end{align}\label{eqn:catalyst-error-seq-s-cnvx}
            Take note that, the error sequence requires knowledge on $F^*$. 
            It's not an issue in theoretical discussion for the convergence analysis and complexity analysis because it's assumed to exist. 
            Unfortunately it's of upmost practical concern because the value $F^*$ is not accessible in practice and depends on the specific context of mathematical model of a practical problem. 
            In the work by Lin et al., the example algorithm given is 
            \todo{Cite this algorithm}
            {\hl{MISO Prox}} which automatically builds a lower bound estimates on $F^*$ in the outer loop. 
            In practice, achieving higher accuracies in $\epsilon_k$ in inner loop would validate the over all convergence claim, but results in slower algorithm. 
            Therefore, we want to know how to achieve $\epsilon_k$ without over compensating. 
            \par
        ...
        \subsubsection{Inner loop complexity}
            The following parts will complement the report and talk about the convergence of the inner loop, and the overall algorithm. 
            \par
            Denote the outer loop algorithm by $\mathbb A$, and the inner loop algorithm by $\mathbb M$. 
            The outer loop iteration algorithm generates $(x_k, y_k)_{k \ge0}$ by Definition \ref{def:lin-catalyst-original} and, for genericity assume that the inner loop is an iterative generating $(z_{k, i})_{i \ge 0}$ where $k$ denotes the iteration index of the outer loop. 
            \par
            Lemma B1, B2 and Appendix item B.4 from Lin et al. \cite{lin_universal_2015} shows the convergence of the inner loop. 
            It requires an error sequence $(\epsilon_k)_{k \ge 0}$ given in prior to assert the convergence rate of the outer loop. 
            With warm starting, it uses the most recent iterate $x_{k - 1}$ (or equivalently some extrapolation using the most recent iterates) to initialize the inner loop to find the next $x_k \in \mathcal J_{F, \kappa^{-1}}^{\epsilon_{k}} y_{k - 1}$.
            Warm starting the inner loop couples its convergence rate with the outer loop, allowing for the overall complexity analysis.  
            Given the error sequence in Equation \ref{eqn:catalyst-error-seq-s-cnvx}, if $\mu > 0$ so the outer loop has linear convergence on $F(x_k) - F^*$, then it demands a linear convergence rate on model function $\mathcal M^{\kappa^{-1}}_F(\cdot, y_{k - 1})$ for the inner loop to an optimal complexity overall. 
            If $\mu = 0$, and $(\epsilon_k)_{k \ge0}$ is given by Equation \ref{eqn:catalyst-error-seq-cnvx}, so the outer loop has $\mathcal O(1/k^2)$ convergence rate on $F(x_k) - F^*$. 
            With the assumption $\argmin_x F(x)$ is a bounded set and the inner loop converges linearly, an optimal overall complexity is achievable. 
            \par 
            ...
            \par
            \todoinline{\noindent
                Add more details about what they actually did in the original paper.
                \begin{enumerate}
                    \item [$\square$] The condition of warm starting in the algorithm and complexity bound of the inner loop. 
                    \item [$\square$] The assumption of bounded level set to show the convergence when objective is not strongly convex. 
                \end{enumerate}
            }
            \par 
    ...
    \subsection{Consequences of the inner loop termination criteria}
        ...
        \todo{
            $\square$ Add paper: ``First-order methods of smooth convex optimization with inexact oracle'' \\
            $\square$ Cite them. 
        }
        {\hl{Devolder et al.}}
