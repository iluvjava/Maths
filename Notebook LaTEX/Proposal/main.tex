...
\section{Catalyst accelerations and future works}\label{sec:catalyst}
    ...
    This section reports major results from literatures concerning the mthod of Catalyst Acceleration. 
    \par
    The notations all sections within this section follow. 
    \begin{assumption}
        Given any $\beta > 0$ and $y \in \RR^n$, and we assume that $F: \RR^n\rightarrow \overline \RR$ is a closed convex function. 
        ...
        We denote $\mathcal J_{\beta^{-1}}$ to be the resolvent operator for subgradient of $F$, and $\mathcal J_{\beta^{-1}}^\epsilon$ to be the inexact resolvent operator. 
        \begin{align*}
            \mathcal J_{\beta^{-1}} y &= 
            \left\lbrace
            \right\rbrace
        \end{align*}
    \end{assumption}
    Inspired by works from Güler \cite{guler_new_1992}, and Rockafellar 1976 \cite{rockafellar_monotone_1976}, Lin \cite{lin_universal_2015} proposed a generic method using the convergence claims of Accelerated proximal point method for first order variance reduced algorithms. 
    \par
    ...
    The section at the end will give future works and extensions of the exisiting Catalyst framework. 
    ...
    \begin{remark}
        \todoinline{\noindent 
            Cite the original Definition in the paper for this one. 
            \begin{itemize}
                \item Give explicit formula for $\alpha_k$
                \item Discuss the optimal choice of $\kappa$ as stated in the paper. 
                \item Elucidate how the sub problem can be resolved. 
            \end{itemize}
        }
    \end{remark}
    Much of the heavy lifting of constructing the algorithms were done through The estimating sequence introduced back in Definition \ref{def:nes-est-seq}. 
