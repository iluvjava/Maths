...
\section{Lin 2015}
    ...
    \end{lemma}
    \begin{remark}
        The lamma plays a key role and it allows Lin to denote his Nesterov's estimating sequence to be for all $k \ge0$: 
        \begin{align*}
            \phi_k(x) := 
...
\subsection{Theorems and claims for accelerated proximal gradient}\label{app:sec:thm-claim-acc-prox-grad}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            \frac{L}{L\alpha_k - \mu}. 
        \end{align*}
        They are matched and therefore we substitute $\xi_k, \tau_k$ into $v_{k + 1} = x_{k + 1} + (1 + \xi_k)^{-1}\gamma_k(x_{k + 1} - x_k)$: 
        \begin{align*}
            v_{k + 1} &= 
            x_{k + 1} + \left(
                1 + \frac{\mu}{L\alpha_k - \mu}
...
\section{Proofs for accelerated PPM}
    \subsection{Exact accelerated PPM}
        ...
        \end{align*}
        Subsituting back the cannonical form, we obtained a recursive definition of the Hessian and gradient of the estimating sequence: 
