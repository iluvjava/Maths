...
\section{Introduction}
    ...
    The meta algorithm uses other first order algorithms to evaluate inexact proximal point method and then performs accelerated PPM based on the outputs of other algorithms, hence the umbrella term: ``meta algorithm''. 
    The innovations here are tracking and controlling the errors made in the inexact PPM using Nesterov's estimating sequence throughout and an algorithm called accelerated MISO-Prox by Lin. 
    Prior to this paper, it was an open question on the conditions required to accelerate incremental method such as stochastic gradient descent can be accelerated. 
...
\section{Lin 2015}\label{sec:lin-2015}
    ...
    \begin{lemma}[Lin's inexact proximal inequality]
    ...
    \begin{definition}[Basic 4WD Catalyst Algorithm]
        ...
        \begin{align*}
            ...
            \text{ such that:  }
                \left\lbrace
                    \begin{aligned}
                        & \dist(\mathbf 0, \partial \mathcal M(\bar x_k; x_{k - 1})) 
                        \le 
...
\section{Proofs for accelerated PPM}
    \subsection{Exact accelerated PPM}
        This setion contains all the proofs for the exact accelerated PPM method. 
        ...
        \begin{proof}   
            ...
            \begin{align*}
                ...
                \\
                    ...
                    \right\rangle. 
            \end{align*}
            Next, to inductively assert the inequality we consider the choice of $y_k$ and scalar values $\alpha_k, \lambda_k$ which set the cross produce term zero, and the coefficient of the norm term larger than zero. 
...
\section{Proofs for Catalyst Meta Acceleration}
    ...
    Recall inexact evaluation $x_k \approx \mathcal J_{\kappa^{-1}}y_{k - 1}$ such that $\mathcal M^{\kappa^{-1}}(x_k; y_{k - 1}) - \mathcal M^{\kappa^{-1}}(\mathcal J_{\kappa^{-1}}; y_{k - 1}) \le \epsilon_k$; $x_k^* = \mathcal J_{\kappa^{-1}}y_{k - 1}$ to be the exact evaluation. 
