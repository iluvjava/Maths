...
\section{Introduction}
    ...
    \subsection{Contributions}
        ...
        \begin{align*}
            ...
            \right). 
        \end{align*}
        Our first main result shows that if $\theta_{k + 1} = (\rho_k\alpha_k(1 - \alpha_k)/(\rho_k\alpha_k^2 + \alpha_{k + 1}))$, using the R-WAPG we proposed (Definition \ref{def:wapg}), we can show that the gap $F(x_k) - F(x^*)$ is bounded by:
        \begin{align*}
            \mathcal O\left(
                \left(
            ...
            \right). 
        \end{align*}
        Our second main result shows that there exists $\rho_k > 1$ such that our R-WAPG reduces perfectly to the a variant proposed in Chambolle Dossal \cite{chambolle_convergence_2015}. 
        And when $\rho_k = 1$, R-WAPG reduces perfectly to V-FISTA by Beck \cite{beck_first-order_2017}, and it produces convergence claim consistent with the literatures. 
        We will also present a simplified convergence rate when $\rho_k \in (0, \alpha_{k - 1}^2), \mu = 0$ which is also consistent with the literatures. 
        \par
...
\section{Formulation of R-WAPG and its convergence rates}
    ...
    \begin{definition}[R-WAPG sequence]\label{def:rwapg-seq}
        ...
        Then the sequence $(\alpha_{k})_{k \ge 1}, (\rho_k)_{k \ge 1}$ satisfy for all $k \ge 2$: 
        \begin{align*}
            &\alpha_k \in (0, 1), 
            \\
            & 0 < \rho_k \alpha_k^2 < 1, 
...
\subsection{R-WAPG describes existing accelerations scheme in the literatures}
    ...
    \begin{lemma}[Specialize convergence claim]\label{lemma:special-rwapg-convergence-claim}
        
    \end{lemma}
    \begin{lemma}[First example sequence]
        Choose any $a \ge 2$. 
        Define $\forall k \ge 1$ the sequence $(\alpha_k)_{k \ge 1}, (\rho_k)_{k \ge 1}$ by 
        \begin{align*}
            \alpha_k &= \alpha/(k + 1), 
            \\
            \rho_k &= \frac{(k + 1)(k + a)^2}{ak(k + a + 1)}. 
        \end{align*}
        Then $\alpha_k, \rho_k$ satisfies Definition \ref{def:rwapg-seq}. 
        It is a valid choice of sequence for the R-WAPG algorithm, which means that they satisfy: 
    \end{lemma}
