...
\section{Introduction}
    ...
    \par 
    A good choice of a minimization algorithm is the Nesterov's acceleration schemes that originally proposed in 1983 \cite{nesterov_method_1983}. Initializing $x_0 = y_0$ the algorithm finds $(x_k)_{k \ge 1}$ and $t_1 = 1$ by 
    \begin{align*}
        & x_{k + 1} = y_k - L^{-1}\nabla F(y_k), 
        \\
    ...
    \par
    Inspired by prior works and the method of Nestrov's estimating sequence \cite{nesterov_lectures_2018}, we present to you an alternative to restarting that performs well empirically and along with it, a unified framework of Accelerated Proximal Gradient (APG) which we call Relaxed Weak Accelerated Proximal Gradient (R-WAPG). 
    The R-WAPG has a convergence claim of $F(x_k)- F(x^*)$ for a much flexible choice of $(\theta_t)_{t \ge 1}$ and the descriptive power to cover several variants of FISTA in the literatures.  
    ...
    \subsection{Contributions}
        ...
        \par
        Firstly, we relaxed the traditional choice of the sequence $\theta_k$ and showed an upper bound of the optimal gap. 
        Let $(\alpha_k)_{k \ge 1}, (\rho_k)_{k \ge 1}$, given any $\gamma_1 \in (0, L]$ and define $\alpha_1$ by 
        \begin{align*}
            ...
            \right). 
        \end{align*}
        We define $\alpha_k$ for $k \ge 2$ by: 
        \begin{align*}
            &\alpha_k \in (0, 1), 
            \\
            & 0 < \rho_k \alpha_k^2 < 1, 
            ...
            \right). 
        \end{align*}
        Our first main results shows that if $\theta_{k + 1} = (\rho_k\alpha_k(1 - \alpha_k)/(\rho_k\alpha_k^2 + \alpha_{k + 1}))$, using the R-WAPG we proposed (Definition \ref{def:wapg}), we can show that the gap $F(x_k) - F(x^*)$ is bounded by:
        \begin{align*}
            \mathcal O\left(
                \left(
            ...
            \right). 
        \end{align*}
        Our second main results shows that there exists $\rho_k > 1$ such that our R-WAPG reduces perfectly to the variants proposed in Chambolle Dossal \cite{chambolle_convergence_2015}. 
        And when $\rho_k = 1, \rho > 0$, R-WAPG reduces perfect to V-FISTA by Beck \cite{beck_first-order_2017} and it produces convergence claim consistent with the literatures. 
        \par
        ...
        The algorithm is parameter free, meaning that it doesn't require knowing $L, \mu$ in advance, and it determines the value of $\theta_t$ by estimating the local concavity using iterates $y_{k}, y_{k + 1}$. 
        We conducted ample amount of numerical experiments to show that it has favorable convergence rate in practice, and it behaves similarly to the FISTA with monotone restart. 
...
\section{Stepwise formulation of weak accelerated proximal gradient}
    ...
    It defines a procedures of generating $x_{k + 1}, v_{k + 1}$ given any $x_k, v_k$. 
    Propsotion \ref{prop:stepwise-lyapunov} states the inequality that describes a decreasing quantity that involves $F(x_k), F(x_{k + 1})$ at each single iteration. 
    ...
    \begin{proof}
        ...
        Here we drop the subscript $k$, $k + 1$ on $\gamma_{k}, \hat \gamma_{k + 1}$ because they are fixed throughout the proof and it makes for better readibility! 
        \par 
...
\section{Formulation of R-WAPG and its convergence rates}
    ...
    \begin{definition}[R-WAPG sequence]\label{def:rwapg-seq}
        Assume $0 \le \mu < L$. 
        For any $\gamma_1 \in (0, L]$, define $\alpha_1 = (1 - \alpha_1)\gamma_1 + \mu \alpha_1$. 
        Then the sequence $(\alpha_{k + 1})_{k \ge 1}, (\rho_k)_{k \ge 1}$ satisfy for all $k \ge 2$
        \begin{align*}
            &\alpha_k \in (0, 1), 
            \\
            & 0 < \rho_k \alpha_k^2 < 1, 
    ...
    \begin{definition}[Relaxed weak accelerated proximal gradient (R-WAPG)]\label{def:wapg}
        ...
        Let $(\alpha_k)_{k \ge1}, (\rho_k)_{k \ge 1}$ be given by Definition \ref{def:rwapg-seq}. 
    ...
    \begin{observation}
        ...
        \par
        The algorithm chains together the sequence of $(\hat \gamma_{k+1})_{k \ge 1}$ and $(\gamma_k)_{k \ge 1}$ in Definition \ref{prop:stepwise-lyapunov} through the equalities: 
        \begin{align*}
            \hat \gamma_{k + 1} = L\alpha_k^2 = (1 - \alpha_k)\gamma_k + \alpha_k \mu
    ...
    \end{proposition}
    \begin{proof}  
        Since $\hat \gamma_2, \gamma_1$ has $\hat \gamma_2 = (1 - \alpha_1)\gamma_1 + \mu \alpha_1$ and $R_2$ is defined in the same manner as in Proposition \ref{prop:stepwise-lyapunov}, we can invoke Proposition \ref{prop:stepwise-lyapunov} and it gives: 
        \begin{align*}
            F(x_{2}) - F(x^*) + R_2 + \frac{L \alpha_k^2}{2}\Vert v_{2} - x^*\Vert^2
            &\le 
            (1 - \alpha_1)\left(
        ...
        \par 
        Next, for all $k = 2, 3, \cdots, N$, the R-WAPG described in \ref{def:wapg} has $\hat \gamma_{k + 1} = L\alpha_{k - 1}^2 = (1 - \alpha_k)\gamma_k + \mu\alpha_k$. 
        We have also $\alpha_k \in (0, 1)$ by Observation \ref{obs:r-wapg-observation-1}. 
        So $\gamma_k, \hat \gamma_{k + 1}, \alpha_k$ fits Definition \ref{def:stepwise-wapg}. 
        The description of $R_k$ is the same as in Proposition \ref{prop:stepwise-lyapunov} hence we can use Proposition \ref{prop:stepwise-lyapunov}, to show that for all $k \ge 1$: 
        {\small
        \begin{align*}
