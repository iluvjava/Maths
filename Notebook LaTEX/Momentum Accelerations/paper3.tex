...
\section{Introduction}
    ...
    \subsection{Contributions}
        ...
        The choices for $\theta_k$ varies for different varints of the accelerated proximal gradient algorithm. 
        ...
        In Chapter 10 of Beck's Book \cite{beck_first-order_2017}, a variant called V-FISTA can achieve the faster linear convergencec rate: $\mathcal O((1 - \sqrt{\mu/L})^k)$ on the optimality gap for $\mu > 0$ strongly convex $F$. 
    ...
    \subsection{Prior works}
        \todoinline{The following are notes to the writer. There is in fact little prior works in this regard. People tried to unify all the variants but all of them are trying to unify the convergence claims under one theorem. Flexible choices of the momentum parameters are the prime focus in heavy ball momentum, or FISTA with Hessian Damping. Their approaches are usually using dynamical system. }
...
\section{R-WAPG describes existing accelerations scheme in the literatures}
    ...
    \begin{lemma}[Constant R-WAPG sequences]\label{lemma:constant-rwapg-seq}
        ...
        \begin{enumerate}
            ...
            \item $\theta = (1 - \mu/L)(1 - \alpha)/(-\mu/L \alpha + 2\alpha - \alpha^2)$. 
            \item If $\alpha = \sqrt{\mu/L}$, then $\rho = 1$, $\theta = (1 - \sqrt{\mu/L})/(1 + \sqrt{\mu/L})$. 
        \end{enumerate}
    \end{lemma}
...
\subsection{The method of Free R-WAPG}
    ...
    {\footnotesize
        ...
        \begin{algorithmic}[1]
        \STATE{\textbf{Input: } $f, g, x_0\in \RR^n, N \in \N$}
        \STATE{\textbf{Initialize: }$x_1:= x_0; y_1 := x_0;L := 1; \mu := 1/2; \alpha_0 = 1;$}
        \FOR{$k = 1, 2, \cdot, N$}
            \STATE{\textbf{Compute: }$\nabla f(x_k); x^+:= [I + L^{-1}\partial g](x_k - L^{-1}\nabla f(x_k))$;}
            \WHILE{$L/2\Vert x^+ - x\Vert^2 < D_f(x^+, x)$}
                \STATE{$L:= L/2$;}
                \STATE{$x^+ = [I + L^{-1}\partial g](x_k - L^{-1}\nabla f(x_k))$; }
            \ENDWHILE
            \STATE{$x_{k + 1} := x^+$;}
            \STATE{$\mu:= \min(L, \mu)$;}
            \STATE{$\alpha_{k} := (1/2)\left(\mu/L - \alpha_{k - 1}^2 + \sqrt{(\mu/L - \alpha_{k - 1}^2)^2 + 4\alpha_{k - 1}^2}\right)$;}
