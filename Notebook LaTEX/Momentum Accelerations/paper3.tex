...
\section{R-WAPG describes existing accelerations scheme in the literatures}
    ...
    \begin{lemma}[Constant R-WAPG sequences]\label{lemma:constant-rwapg-seq}
        Suppose $(\alpha_k)_{k \ge 0}, (\rho_k)_{k \ge 0}$ are R-WAPG sequences given by Definition \ref{def:rwapg-seq}, and they are constant meaning that there exists $\rho, \alpha$ such that $\alpha_k := \alpha$ and $\rho_k := \rho$ for all $k \ge 0$.
        Define the momentum term $\theta = \rho\alpha(1 - \alpha)/(\rho\alpha^2 + \alpha)$. 
        Then the constant R-WAPG sequence satisfies: 
        \begin{enumerate}
            \item $\rho = (1 - \mu/L\alpha^{-1})/(1 - \alpha)$. 
            \item $\theta = (1 - \alpha)(1 - \mu/L \alpha^{-1})/(1 - \mu/L)$. 
    ...
    \begin{proof}
        ...
        To see (iv), substitute $\alpha = \sqrt{\mu/L}$ for $\rho$. 
