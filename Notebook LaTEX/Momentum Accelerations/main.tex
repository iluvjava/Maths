...
\section{The Proximal Point Method in the Convex Case}
    ...
    \subsection{The Lyapunov function of Convex PPM}
        ...
        \begin{theorem}[Convergence Rate of PPM (Strongly Convex)]
            
            
        \end{theorem}
\section{Simple of the Proximal Point Method}
    ...
    \subsection*{Generic Gradient Descent}
        ...
        \begin{theorem}[Generic Approximated PPM]\label{thm:lower_approx_ppm_convergence}
            With $f$ be a function that has minimizer: $x_*$; $l_f(x; x_t)$ be a convex, lsc, proper lower-bounding function, define lower-bounding $\phi_t = \eta_{t + 1}l_f(x; x_t)$. 
            ...
            $$ 
            Fix any $x_0$, assume that the iterates $x_t$ defined for $t\in \NN$ satisfies
            \begin{align*}
            ...
            \end{align*}
            And for all $\eta_{t} \in (\epsilon, 2L^{-1} - \epsilon)$ with $\epsilon > 0$, the algorithm has sublinear convergence rates of
            \begin{align*}
                \frac{L - \epsilon^{-1}}{TL\epsilon} &\le 
        ...
        \begin{remark}
            We speculate based on observing that the inequality 
            $$
                \phi_t(x) \le \eta_{t + 1}f(x) \le \phi_t(x) 
                + 
                \frac{L\eta_{t + 1}}{2}\Vert x - x_t\Vert^2 \quad \forall x \in \RR^n, 
            $$
            was invoked with $x = x_{t + 1}$ for the PPM descent inequality, meaning that having $\forall x \in \RR^n$ can be relaxed into $\forall (x_t)_{t \in \NN}$ generated by the algorithm, $\exists (L_t)_{t \in \NN}$, 
            $$
                \phi_t(x) \le \eta_{t + 1}f(x) \le \phi_t(x) 
                + 
            ...
            This can be achieved by choosing the function $\Phi_{t + 1}$ at each iteration smartly. 
            \par
