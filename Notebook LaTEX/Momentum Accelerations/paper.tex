...
\label{sec:generic_ag_ppm_lyapunov_analysis}
    ...
    The Lypunov function in use is identical to Ahn and Sra \cite[section 4.2]{ahn_understanding_2022} except for the involvement of a gradient mapping. 
    We present the results differently by compatifying results into 
    ...
    states new results that are extension to Ahn and Sra's works by introducing an error term to the Lypunov analysis, allowing for a weaker constraints for the stepsize paramters $\tilde \eta_i, \eta_i$ from 
    \hyperref[def:ag_prox_grad_generic]{definition \ref*{def:ag_prox_grad_generic}}. 
    \par
    Theorem \ref*{thm:ag_generic_stepsize_constrants} states the weakened constraints on the stepsize parameters $\tilde \eta_i, \eta_i$. 
