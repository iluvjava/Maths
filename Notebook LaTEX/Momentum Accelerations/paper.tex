...
\section{Scratch paper stuff}
    ...
    \subsection{The line search algorithm idea}
        ...
        The ODE understand provoked interest in the restart techniques of AG/FISTA and the approaches are diverse and fruitful. 
        ...
        \begin{algorithm}
            ...
            \begin{algorithmic}[1]
                \Procedure{Routine1}{$L, h=f+g, x_0 = y_0 = z_0, \epsilon > 0$} %\Comment{The gcd of $a$ and $b$}
                    \State \textbf{Initialize} $\tilde \mu = 0, \eta_0 = 0, \tilde \eta_1 = L^{-1}$
                    \For{$t = 1, \cdots, N$}
                        \State $\tilde x \gets (\tilde \mu\tilde \eta_{t + 1} + 1)^{-1}(\tilde \mu \tilde \eta_{t + 1}y_t + x_t - \tilde \eta_{t + 1}\mathcal G_l(y_t))$
                        \If{$\exists \tilde \mu \in [\epsilon, L]: l_h(\tilde x, x_t, \tilde \mu) \le h(\tilde x)$}
                            \State \textbf{Find } $\tilde \mu \in [\epsilon, L]$ s.t: $l_h(\tilde x, x_t, \tilde \mu) \le h(\tilde x)$ \Comment{Use binary search}
                            \State $\tilde \eta_{t + 1} \gets (\sqrt{L\tilde \mu} - \tilde \mu)^{-1}$
                        \Else \Comment{$\tilde \mu = 0$}
                            \State $\tilde \mu \gets 0$
                            \State $\tilde \eta_{t + 1} \gets \theta_tL^{-1}$
                        \EndIf
            ...
            \end{algorithmic}
        \end{algorithm}
        The quantity $\epsilon$ is the tolerance for line search to terminate if it can't find any such value. 
