...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    \textcolor{purple}{
        ...
        \begin{enumerate}[nosep]
            ...
            \item Since $\mu = 0$, by Proposition \ref{prop:wapg-first-equivalent-repr}, the extrapolated iterate has $y_k = x_{k + 1} + \alpha_{k + 1}(\alpha_k^{-1} - 1)$
        \end{enumerate}
        ...
        \end{align*}
        It remains to verify whether the sequence $\alpha_k$ is still valid, by which it means that $\alpha_k \in (0, 1)$ for all $k \ge 1$. 
        We will soon reveal that it will be true in the part that comes. 
    }
    \par
    ...
    \begin{algorithm}
        ...
        \FOR{$k = 0, 1, 2, \cdots, N$}
            ...
            \STATE{$\theta_{k + 1} := \alpha_{k + 1}(\alpha_k^{-1} - 1)$;}
    ...
    \subsection{Numerical experiments}
        In this section, we will show the numerical experiments results of algorithm (Algorithm \ref{alg:free-rwapg}) in this section then, we compare with other APG algorithms in the literatures: the V-FISTA, and FISTA algorithm described in Section (10.7.7, 10.7.6) by Beck \cite{beck_first-order_2017}.
        ...
        \par
        We will now introduce the global setup of our numerical experiments. 
        To compare the convergence of different algorithms, we measure the logarithm of the relative optimaity gap $\delta_k$ which we defined: 
        ...
        \begin{align*}
            ...
            \right). 
        \end{align*}
        We give the same inigial guess $x_0$ for all algorithms.
        Since $F^*$ is not always known in prior, we used the minimum of all $F(x_k)(1 - 2^{53})$ across all algorithms to estimate the minimum $F^*$ when it's not avaiable. 
        \par
        Fix some $\epsilon > 0$. 
        The termination condition is $\Vert \mathcal G_L(y_k)\Vert < \epsilon$ for all algorithms.
        We implemented line search for Lipschitz smoothness constant $L$ only for FISTA. 
        For FR-WAPG and V-FISTA, an over estimate of the Lipschitz modules is given. 
        \subsubsection{Experiments with a simple convex quadratic function}
            Consider $\min_{x \in \RR^n} \{F(x):= f(x) + 0\}$ given by $f(x) = (1/2)\langle x, A x\rangle$. 
            Here, $A$ is set to be a positive semi-definite diagonal matrix, hence problem is a convex quadratic and one of its minimizer is $x^* = \mathbf 0$,  with minimum being zero.
            We repeat the same experiment for Algorithm \ref{alg:free-rwapg}, M-FISTA, and V-FISTA for 30 times with different initial guesses. 
