...
\begin{abstract} 
    ...
    A much weaker condition of extrapolation parameters in $y_{k + 1} = x_{k + 1} - \theta_k(x_{k + 1} - x_k)$ is precisely characterized to show an upper bound on the optimality gap $F(x_k) - F^*$ using a generic algorithm R-WAPG (Relaxed Weak Accelerated Proximal Gradient) we proposed. 
    ...
    Inspired by a small detail in the proof, explorative numerical experiments were conducted to show the convergence of R-WAPG where the extrapolation sequence is generated using Bregman Divergence of $f$ on sucessive iterate $y_{k + 1}, y_{k}$. 
...
\section{Equivalent representations of R-WAPG}\label{sec:rwapg-equiv-repr}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            &= \frac{\rho_k\alpha_k(1 - \alpha_k)}{\rho_k\alpha_k^2 + \alpha_{k + 1}}.
        \end{align*}
        Going from the left to right on the fourth equality, we used $L\alpha_{k + 1}^2 = (1 - \alpha_{k + 1})\rho_kL\alpha_k^2 + \mu \alpha_{k + 1}$ established by: 
        \begin{align*}
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    Assuming inductively that $q = \tilde \mu_k/L$,  $\alpha_k \in (0, 1)$, then 
    \begin{align*}
        \alpha_{k + 1} 
        &= 
        (1/2)\left(
