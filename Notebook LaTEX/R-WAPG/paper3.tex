...
\section{Stepwise formulation of weak accelerated proximal gradient}\label{sec:stepwise-stuff}
    ...
    \begin{proposition}[Stepwise Lyapunov]\label{prop:stepwise-lyapunov}\;\\
        ...
        With $\epsilon_k$ given by \eqref{eqn:regret}, define: 
...
\section{R-WAPG and its convergence rates}\label{sec:rwapg-formulation-convergence}
    ...
    \begin{proposition}[R-WAPG convergence claim]\label{prop:wapg-convergence}\; \\
        ...
        Let $\epsilon_k$ be given by \eqref{eqn:regret}. 
    ...
    \subsection{Equivalent representations of R-WAPG}
        ...
        \begin{definition}[R-WAPG momentum form]\label{def:r-wapg-momentum-form}
            ...
            \begin{tcolorbox}
                For $k=1, 2,\ldots $
                \begin{align*}
                    & x_{k + 1} \defeq y_k - L^{-1}\mathcal G_Ly_k, 
                    \\
                    & 
                    y_{k + 1} \defeq 
                    x_{k + 1} + 
...
\section{R-WAPG unifies existing accelerations scheme}\label{sec:rwapg-literatures}
    ...
    \begin{remark}
        ...
        The above theorem gives convergence rate for a much relax choice of momentum parameter $\theta$ generalizing the convergence of constant step size FISTA to all convex stronlgy convex function and all possible $\alpha_k \in (\mu/L, 1)$. 
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    To prvent the case $\alpha_k = 1$, we may always force the esimate $\mu = L/2$ whenever $\mu > L/2$. 
    ...
    \subsection{Numerical experiments}
        ...
        \subsubsection{Simple convex quadratic}
            ...
            Figure \ref{fig:simple-quadratic-r-wapg-mu-estimates} shows the estimated $\mu_k$ of FR-WAPG on the left and the convergence rate of the test algorithms on the right for one specifice initial condition. 
        ...
        \subsubsection{LASSO}
            ...
            In addition, an experiment were conducted to show the estimate $\mu_k$ and optimality gaph $F(x_k) - F^*$ in Figure \ref{fig:single-lass-mu-estimates}. 
            \begin{figure}[H]
                \begin{subfigure}[b]{0.47\textwidth}
...
\section{Discussions}
    This paper provides the results of an upper bound on the optimality gap $F(x_k) - F^*$ for smooth plus nonsmooth composite optimization problem with convex objectives where under the much weaker assumption for the momentum parameter specified by Definition \ref{def:rwapg-seq}. 
