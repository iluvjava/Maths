...
\begin{abstract} 
    ...
    In addition, we also study the convergence rate of strongly convex objective with a constant momentum term. 
...
\section{Introduction}
    \subsection{Motivations}
        ...
        \todoinline{
            $\blacksquare$: Improves the motivation parts here by starting without the math and then move onto the mathematics. 
        }
...
\section{Preliminaries}\label{sec:preliminaries}
    ...
    Using $\overline \RR$ we model a general convex function $g: \RR^n \rightarrow \overline{\RR}$ as a function on $\RR$.  
...
\section{Stepwise formulation of weak accelerated proximal gradient}\label{sec:stepwise-stuff}
    ...
    \begin{proof}
        ...
        \end{enumerate}
        \todoinline{
            $\blacksquare$: Make it clear that we are going to prove the end results here from the listed intermediate results. 
        }
...
\section{R-WAPG and its convergence rates}\label{sec:rwapg-formulation-convergence}
    ...
    \todoinline{\noindent
        $\blacksquare$: This definition below doesn't directly contribute to the convergence proof, the recurrence relation is more important to the convergence proof. The constraints on the sequence is used to show the equivalent representations. 
    }
...
\section{Equivalent representations of R-WAPG}\label{sec:rwapg-equiv-repr}
    ...
    Sometimes the same algorithm include two iterates $(x_k, y_k)$, sometimes they inlcude three iterates $(x_k, y_k, z_k)$. 
    ...
    \subsection{Proofs of equivalent representations of R-WAPG}
        ...
        \begin{proof}
            ...
            \end{align}
            The coming parts simplifies $y_k$ firstly for $k = 1$, then for all $k \ge 2$. 
...
\section{R-WAPG unifies existing accelerations scheme}\label{sec:rwapg-literatures}
    ...
    \todoinline{\noindent
        $\blacksquare$: Explain what this table shows. 
    }
    \begin{table}[H]
        \centering
        {\scriptsize
    ...
    \end{remark}
    \todoinline{
        $\blacksquare$: Add some explanations here to brief the readers on what is about to come. 
    }
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    \par 
    Sections of $(\alpha_k)_{k \ge 0}$ generated by Algorithm \ref{alg:free-rwapg} can be an R-WAPG sequence making the convergence result (Proposition \ref{prop:wapg-convergence}) of R-WAPG is relevant to a limited extend. 
    For the full sequence $(\alpha_k)_{k \ge0}$ to be an R-WAPG sequence requires additional assumption that none of the estimate has $\tilde \mu = L$. 
    Let $\tilde \mu_k$ be an estimate produced by the algorithm and assume that it satisfies that $\tilde \mu_k/ L \in [0, 1)$ and $\alpha_k \in (\mu/L, 1)$.
    ...
    \subsection{Numerical experiments}
        ...
        \subsubsection{Simple convex quadratic}
            ...
            \todoinline{\noindent
                $\blacksquare$: Another important feature of the numerical results is the hybrid behavior of the algorithm. 
            }
            \par
            ...
            One significant feature here is that FR-WAPG initially behaves similar to M-FISTA, but as the iteration goes on, it started to behave like V-FISTA. 
            The second surprising feature is the monotone descent it shows on the graph visually. 
            Unfortunately it's numerically verified that the method is not monotone in general, and it just looks monotone on the graph. 
            \begin{figure}[H]
