...
\end{abstract}
\todo{Remember to update these subject classificiations. }
{
...
\section{Introduction}
    \subsection{Motivations}
        ...
        However, things are not sunshine and rainbow because the above scheme is associated with the following issue: 
        If $F$ is $\mu > 0$ strongly convex, the optimality gap $F(x_k) - F(x^*)$ oscillates and converges sub-linearly, making it worse than gradient descent. See page 9 of Su et al. \cite{su_differential_2016}. 
        \par
        ...
        \subsection{Prior works}
            \todoinline{\noindent
                The prior works' section is too little, it requires some expansion in the future for sure.
                \begin{enumerate}
                    \item Paper: ``An introduction to continuous optimization for imaging'', Chambolle, Pock. 
                \end{enumerate}
                
            }
            \todo{$\blacksquare$ Add them \\ $\blacksquare$ Cite them.}{\hl{Apidopoulos et al. }\cite{apidopoulos_convergence_2018}}
            showed that a relaxed choice of the sequence that doesn't strictly follow the Nesterov's rule still leads to the convergence of the momentum algorithm. 
            Besides restarting, \todo{$\blacksquare$ Add them \\ $\blacksquare$ Cite them. }{See Attouch, et al. \cite{attouch_first-order_2022}, Maulen and Peypouquet \cite{maulen_speed_2023}} and references within.
    ...
    \subsection{Contents}
        ...
        Section \ref{sec:rwapg-formulation-convergence} formulates the full R-WAPG algorithm and characterize conditions for R-WAPG sequence sufficient to derive the optimality upper bound in Proposition \ref{prop:wagp-convergence}. 
        Section \ref{sec:rwapg-equiv-repr} gives 3 equivalent representations of the R-WAPG algorithms that are comparable to instances of Accelerated Proximal Gradient algorithm found in the literatures. 
        Section \ref{sec:rwapg-literatures} formulates FISTA, and V-FISTA sequences and proves the convergence rate of several variants  FISTA using the R-WAPG algorithm and its convergence rate developed in the previous sections. 
        Finally, in Section \ref{sec:free-rwapg} gives a formulation of a parameter free version of R-WAPG algorithm and showcase the numerical experiments for regression, LASSO, and logistic regression. 
...
\section{Stepwise formulation of weak accelerated proximal gradient}\label{sec:stepwise-stuff}
    ...
    \begin{lemma}[A useful lemma]\label{lemma:ineq-q3}
    ...
    \end{lemma}
    \begin{proof}
        % Recall \eqref{eqn:Q1}, \eqref{eqn:Q2} from 
    ...
    \begin{proposition}[Stepwise Lyapunov]\label{prop:stepwise-lyapunov}\;\\
        ...
        \begin{align}
            ...
            \left(
                ...
                \Vert v_k - y_k\Vert^2
            \right). 
        \end{align}\label{eqn:stepwise-lya-1}
    ...
    \begin{proof}
        ...
        \par 
        We started with $F(x_{k + 1}) + R_{k + 1}$ as Equation \eqref{eqn:stepwise-lya-proof-eqn1}, and $\hat \gamma/2\Vert v_{k + 1} - x^*\Vert^2$ as Equation (\ref{eqn:stepwise-lya-proof-eqn2}). 
        In the proof that follows, we show (\ref{eqn:stepwise-lya-proof-eqn2}) equals to \eqref{expr:stepwise-lya-expr2.2}. 
        We add (\ref{eqn:stepwise-lya-proof-eqn2}), (\ref{expr:stepwise-lya-expr2.2}) to produce \eqref{eqn:stepwise-lya-proof-eqn3}. 
        Then we show \eqref{eqn:stepwise-lya-proof-eqn3} equals to \eqref{eqn:stepwise-lya-proof-eqn3.1}. 
        Finally, the entire LHS of \eqref{ineq:stepwise-lya-2} equals to the RHS of \eqref{ineq:stepwise-lya-proof-ineq4}. 
        Going from \eqref{eqn:stepwise-lya-proof-eqn3.2} to \eqref{ineq:stepwise-lya-proof-ineq4}, we used \ref{thm:prox-grad-ineq} which is all we need, and it is the same as the inequality that we want to prove. 
        \par
        ...
        \begin{align}
            ...
            &\quad 
                ...
                \left(
                    ...
                    \Vert v_k - y_k\Vert^2
                \right). 
            \label{eqn:stepwise-lya-proof-eqn1}
        \end{align}
        ...
        \begin{align} 
            ...
            \end{split}
            %\tag{2.2*}
        \label{expr:stepwise-lya-expr2.2}
        \end{align}
        }
        ...
        \end{split}
        %\tag{3*}
        \label{eqn:stepwise-lya-proof-eqn3}
        \end{align}
        ...
        \end{split}
            % \tag{3.1*}
            \label{eqn:stepwise-lya-proof-eqn3.1}
        \end{align}
        ...
        \begin{align}
            ...
            \end{split}
        % \tag{3.2*}
        \label{eqn:stepwise-lya-proof-eqn3.2}
        \end{align}
        ...
        \end{split}
        \label{ineq:stepwise-lya-proof-ineq4}
        \end{align}
        }
...
\section{Formulation of R-WAPG and its convergence rates}\label{sec:rwapg-formulation-convergence}
    ...
    \begin{proof}  
        ...
        Observe that $R_2$ is defined in the same manner as in Proposition \ref{prop:stepwise-lyapunov} with $k = 1$, we can invoke Proposition \ref{prop:stepwise-lyapunov}, and it gives: 
        \begin{align*}
    ...
    \end{proof}
    \begin{remark}        
        The choice of $\rho_k = 0$ reproduces traditional choice of sequences for the APG algorithm. 
...
\section{Equivalent representations of R-WAPG}\label{sec:rwapg-equiv-repr}
    ...
    \subsection{A list of equivalent representations of R-WAPG}
        ...
        \begin{remark}
            ...
            \todo{$\blacksquare$ Add them \\ $\blacksquare$ Cite them. }
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    \subsection{Numerical experiments}
        ...
        \subsubsection{LASSO}
            This section present results of numerical experiment for solving the (least absolute shrinkage and selection operator) LASSO problem proposed by \hl{Tibshirani} \cite{tibshirani_regression_1996}. 
