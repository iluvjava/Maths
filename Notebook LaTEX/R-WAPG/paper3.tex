...
\begin{abstract} 
    \noindent
    We analyze the accelerated proximal gradient method where the sequence that controls the momentum term is much weaker than the Nesterov's rule. 
    Our setting is the convex optimization problem $F^* = \min_x\{f(x) + g(x)\}$ where $f$ is $L$ Lipschitz smooth and $\mu\ge 0$ strongly convex and $g$ is closed and convex. 
    The new generic algorithm R-WAPG (Relaxed Weak Accelerated Proximal Gradient) shows an upper bound on the optimality gap $F(x_k) - F^*$ for any arbitrary choices of extrapolation parameters of updating $y_{k + 1} = x_{k + 1} - \theta_k(x_{k + 1} - x_k)$. 
    R-WAPG is a generic algorithm that has a has one convergence claim that unifies the convex and strongy convex cases. 
    R-WAPG can describes major Euclidean variants of the FISTA. 
    We showed convergence results for several notable variants of FISTA found in the literatures. 
    Inspired by a small detailed in the proof, explorative numerical experiments were conducted to show the convergence of R-WAPG where the momentum sequence is produced by using Bregman Divergence of $f$ on sucessive iterate $y_{k + 1}, y_{k}$. 

\end{abstract}
\todo{Remember to update these subject classifications. }{
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    Let $q = \tilde \mu_k/L$, inductively assume that $\alpha_k \in (0, 1)$, then 
    \begin{align*}
        \alpha_{k + 1} 
        &= 
        (1/2)\left(
    ...
    It is not hard to see that $\alpha_{k + 1} > 0$ as well. 
...
\section{Discussion}
    ...
    Our numerical experiments are explorative, and our theories are our best attempts at making sense of what is happening, and we did it directly with elementary algebra. 
    Nonetheless, two things are clear from the numerical experiments. FR-WAPG performs better than V-FISTA on the first hundreds of iterations, and it doesn't require prior knowledge on parameter $L, \mu$. 
    \par
    ...
    The difficulty here is that it's impossible to know the sequence $(\alpha_k)_{k \ge 0}$ without running the algorithm. 
    It would require more knowledge about the trajectory taken for a specific initial guess. 
    \par 
    From a practical point of view, the numerical experiments show that at the start of the few iterations, the estimate for $\mu$ can be overestimated.
    This is partially explained by the remark of Proposition \ref{prop:stepwise-lyapunov}. 
    It also showed that the function value gives very similar charactercteristics of M-FISTA. 
