...
\section{Introduction and background}
    ...
    \subsection{Our discoveries and inspirations}
        \begin{frame}{Relaxed Weak Accelerated Proximal Gradient}
            ...
            \begin{tcolorbox}
                How much can we relax the choice of the sequence $\theta_k$ and retain an upper bound for the convergence rate? 
            \end{tcolorbox}
    ...
    \subsection{Numerical experiment, LASSO}
        \begin{frame}{The LASSO problem}
            ...
            \begin{enumerate}
                ...
                \item Let $F_{\min}$ be the smallest value of $F(x_k)$, for all algorithms, over all trials, we estimate $F^+$ by $F^+ = F_{\min}*(1 - 2^{53})$. 
            \end{enumerate}
        \end{frame}
        \begin{frame}{LASSO Results plotted}
            With $A\in \RR^{N\times N}$ fixed, for each $x_0 \sim \mathcal N(I, \mathbf 0)$ realized, 20 experiments are preformed and the medium, minimum and maximum of $\delta_k$ are recorded for M-FISTA, Free R-WAPG, and V-FISTA. 
            \begin{figure}[H]
                \begin{subfigure}[b]{0.47\textwidth}
                    \centering
