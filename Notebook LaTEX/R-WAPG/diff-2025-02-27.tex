...
%DIF DEL paper3.tex                    Thu Feb 27 14:50:46 2025
%DIF ADD relaxed-wapg-27-02-2025.tex   Thu Feb 27 21:57:50 2025
...
\title{{\fontfamily{ptm}\selectfont
        ...
        Relaxed Weak Accelerated Proximal Gradient \DIFdelbegin \DIFdel{method}\DIFdelend \DIFaddbegin \DIFadd{Method}\DIFaddend : a \DIFdelbegin \DIFdel{unified framework }\DIFdelend \DIFaddbegin \DIFadd{Unified Framework }\DIFaddend for Nesterov's \DIFdelbegin \DIFdel{acceleration
    }\DIFdelend \DIFaddbegin \DIFadd{Accelerations
    }\DIFaddend }}
...
\date{\today}
\maketitle
\todoinline{Changes accepeted. }

% \vskip 8mm

\begin{abstract}
    \noindent
    \DIFdelbegin \DIFdel{We study the }\DIFdelend \DIFaddbegin \DIFadd{This paper is devoted to the study of }\DIFaddend accelerated proximal gradient \DIFdelbegin \DIFdel{method }\DIFdelend \DIFaddbegin \DIFadd{methods }\DIFaddend where the sequence that controls the momentum term doesn't follow \DIFdelbegin \DIFdel{the }\DIFdelend Nesterov's rule.
    \DIFdelbegin \DIFdel{The paper proposes Relaxed Weak Accelerated Proximal Gradient}\DIFdelend \DIFaddbegin \DIFadd{We propose a relaxed weak accelerated proximal gradient }\DIFaddend (R-WAPG) \DIFaddbegin \DIFadd{method}\DIFaddend , a generic algorithm that unifies the convergence results for strongly convex and convex problems where the extrapolation constant is characterized by a sequence that is much weaker than \DIFdelbegin \DIFdel{the }\DIFdelend Nesterov's rule.
    \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Our }\DIFaddend R-WAPG provides a unified framework for several notable Euclidean variants of FISTA and \DIFdelbegin \DIFdel{verify their convergence}\DIFdelend \DIFaddbegin \DIFadd{verifies their convergences}\DIFaddend .
    In addition, we \DIFdelbegin \DIFdel{also }\DIFdelend provide the convergence rate of strongly convex objective with a constant momentum term.
    We \DIFaddbegin \DIFadd{also }\DIFaddend reformulate R-WAPG so that it doesn't require any parameter, without using the idea of restarting, which we call ``Free R-WAPG".
    Explorative numerical experiments were conducted to show its competitive \DIFdelbegin \DIFdel{convergence}\DIFdelend \DIFaddbegin \DIFadd{advantages}\DIFaddend.
    \todoinline{Changes accepeted}
\end{abstract}
...
\DIFaddend Primary 90C30, 90C25, 65K10; Secondary 65Y20.
\\{\bfseries Keywords: }
...
\section{Introduction}
    \subsection{Motivations}
        ...
        \todo{Changes accepeted. }{
            See \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
            \cite{necoara_linear_2019}}\hskip0pt%DIFAUXCMD
            \mbox{%DIFAUXCMD
            \cite{aujol_parameter-free_2024}}\hskip0pt%DIFAUXCMD
            and references within }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
            \cite{aujol_parameter-free_2024}}\hskip0pt%DIFAUXCMD
            , \mbox{%DIFAUXCMD
            \cite{necoara_linear_2019} }\hskip0pt%DIFAUXCMD
            and references therein }\DIFaddend
        }
        for recent advancements in restarting accelerated proximal gradient algorithm.
        \par
    ...
    \subsection{Contributions of this paper}
        ...
        \DIFaddbegin 
            \DIFadd{and }
        \DIFaddend
        } Dossal \cite{chambolle_convergence_2015}, it has $t_k = (n + a - 1)/a$ for all $a > 2$ which gives weak convergence of the iterates $x_k$ in Hilbert space.
        ...
        \par
        We relax the traditional choice of the sequence $\theta_k$ in Equation 
        \todo{changes accepeted}
        {
            \DIFdelbegin 
                \DIFdel{\ref{eqn:example-algorithm} }
            \DIFdelend \DIFaddbegin 
            \DIFadd{
                \eqref{eqn:example-algorithm} 
            }\DIFaddend 
        }
        and showed an upper bound of the optimal gap.
        ...
        \begin{align*}
            ...
            \right).
        \end{align*}
        Our second main result shows that for any $\alpha_k \ge 0$ the choice of sequence $\alpha_k = a/(a + k)$ results in $\rho_k > 1$ for all $k \in \N$ such that R-WAPG reduces to a variant of FISTA proposed in Chambolle 
        \todo{changes accepeted}
        {
            \DIFdelbegin 
                \DIFdel{, }
        ...
        Dossal \cite{chambolle_convergence_2015}, and we are able to show the same convergence rate in Theorem \ref{thm:r-wapg-on-cham-doss}.
        ...
        The algorithm is parameter free, meaning that it doesn't require knowing $L, \mu$ in advance, and it determines the value of 
        \todo{changes accepeted}
        {
        \DIFdelbegin 
            \DIFdel{$\theta_t$ }
        ...
        by estimating the local concavity using iterates $y_{k}, y_{k + 1}$ from the Bregman divergence of $f$ with minimal computational cost.
        ...
        \todo{changes accepted}
        {   \DIFdelbegin 
            \DIFdel{Section \ref{sec:stepwise-stuff} states and proves Proposition \ref{prop:stepwise-lyapunov}, an }
            \DIFdelend 
            \DIFaddbegin 
            \DIFadd{
                In Section~\ref{sec:preliminaries}, we formulate the composite optimization
                problem considered in the paper and give a proximal inequality connecting the objective function and the proximal gradient
                operator and gradient mapping.
                Section \ref{sec:stepwise-stuff} proves 
                %DIF > Proposition \ref{prop:stepwise-lyapunov}, 
                a key 
            }\DIFaddend 
        } inequality based on a single generic iterative step that acts similar to a descent lemma.
        ...
        \todo{changes accepted}
        {
            \DIFdelbegin 
            \DIFdel{
                Proposition \ref{prop:wapg-convergence} states the convergence rate of R-WAPG through Proposition \ref{prop:stepwise-lyapunov} }
            \DIFdelend 
            \DIFaddbegin 
            \DIFadd{
                Using Proposition \ref{prop:stepwise-lyapunov} from Section~\ref{sec:stepwise-stuff} 
            }
            \DIFaddend
        }
        ...
        \todo{changes accepeted}{
            \DIFaddbegin \DIFadd{, 
                %DIF > Proposition \ref{prop:wapg-convergence} states 
                we provide the convergence rate of R-WAPG}
            \DIFaddend.
            Section \ref{sec:rwapg-equiv-repr} 
            \DIFdelbegin 
                \DIFdel{gives }
            \DIFdelend 
            \DIFaddbegin 
        ...
        three equivalent representations of the R-WAPG algorithms that are comparable to instances of APG found in the literatures.
        ...
        \DIFaddbegin 
        \DIFadd{This }\DIFaddend section proves the convergence rate of several variants of FISTA using the equivalent forms introduced in Section \ref{sec:rwapg-equiv-repr} and the convergence rate developed Section \ref{sec:rwapg-formulation-convergence}.
        Finally, Section \ref{sec:free-rwapg} gives a 
        \todo{changes accepeted}
        {
            \DIFdelbegin 
                \DIFdel{formulation of a }
            \DIFdelend 
            parameter free version of R-WAPG algorithm and showcase 
            \DIFdelbegin 
                \DIFdel{the }
            \DIFdelend 
            \DIFaddbegin 
                \DIFadd{its }
            \DIFaddend numerical experiments for regression, LASSO.
        }


\section{Preliminaries}\label{sec:preliminaries}
    Throughout, we make the following assumption unless specified.
    \DIFdelbegin %DIFDELCMD < \begin{assumption}[Smooth nonsmooth additive]%%%
...
\section{Stepwise formulation of weak accelerated proximal gradient}\label{sec:stepwise-stuff}
    ...
    \end{lemma}
    \begin{proof}
        The proof starts by showing that \eqref{eqn:Q1}
        \todo{changes accepeted}
        {
            \DIFdelbegin 
                \DIFdel{, \eqref{eqn:Q2} follows from \ref{eqn:stepwise-wapg-eqn2}}
            \DIFdelend    
            \DIFaddbegin 
            \DIFadd{
                and \eqref{eqn:Q2} below follow from \eqref{eqn:stepwise-wapg-eqn2}
            }
            \DIFaddend
        }, then the desire results follows naturally using algebra.
    ...
    \DIFaddend \begin{proposition}[Stepwise Lyapunov]\label{prop:stepwise-lyapunov}\;\\
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            {
                \DIFdelbegin 
                \DIFdel{Step 3: adding }
                \DIFdelend 
                \DIFaddbegin 
                    \DIFadd{Adding }
                \DIFaddend 
            }
            both side of Equations \eqref{eqn:stepwise-lya-proof-step-1}, \eqref{eqn:stepwise-lya-proof-step-2}, it follows:
...
\section{Equivalent representations of R-WAPG}\label{sec:rwapg-equiv-repr}
    ...
    {
        \DIFdelbegin 
            \DIFdel{To start , the following proposition }
        \DIFdelend 
        \DIFaddbegin 
            \DIFadd{We start with the following result }
        \DIFaddend 
    }
    on ``abstract similar triangle form'' 
    \todo{changes accepeted}
    {
        \DIFdelbegin 
        \DIFdel{were made to simplify arguments and notations and to make cleaner proofs}
        \DIFdelend 
        \DIFaddbegin 
            \DIFadd{to make later proofs cleaner}
        \DIFaddend.
    }
...
\section{R-WAPG unifies existing accelerations scheme}\label{sec:rwapg-literatures}
    ...
    Last column shows the convergence rate on $F(x_k) - F^*$ 
    \todo{changes accepeted}{
        \DIFdelbegin 
            \DIFdel{which is }
        \DIFdelend 
        \DIFaddbegin 
        \DIFadd{(}\DIFaddend the optimality gap\DIFdelbegin 
            \DIFdel{and a }
        \DIFdelend 
        \DIFaddbegin 
            \DIFadd{) and our }
        \DIFaddend theorem that proves the claim.
    }
    \begin{table}[H]
        \centering
        {\scriptsize
    ...
    \end{proof}
    \begin{remark}
        The sequence $t_k$ is exactly the same as in Chambolle 
        \todo{changes accepeted}
        {
            \DIFdelbegin 
            \DIFdel{, Dossal 
                \mbox{%DIFAUXCMD
                \cite[Theorem 3.1, 8]{chambolle_convergence_2015}}\hskip0pt %DIFAUXCMD
            }
            \DIFdelend 
            \DIFaddbegin 
            \DIFadd{and Dossal 
                \mbox{%DIFAUXCMD
                \cite[Theorem 3.1]{chambolle_convergence_2015}}\hskip0pt %DIFAUXCMD
            }
            \DIFaddend.
        }
    \end{remark}
...
\section{The method of Free R-WAPG}\label{sec:free-rwapg}
    ...
    \subsection{Numerical experiments}
        ...
        \subsubsection{LASSO}
            ...
            {
                \DIFdelbegin 
                \DIFdel{Make the important observation }
                \DIFdelend \DIFaddbegin 
                \DIFadd{An important observation is }
                \DIFaddend 
            }
            that $\mu_k$ didn't converge to the true value in this case, however it didn't affect the convergence of $F(x_k) - F^*$.
