...
\section{Introduction}
    ...
    \subsection{Preliminaries}
        ...
        \end{proof}
        \begin{fact}[Fenchel Rockafellar duality]
    ...
    \subsection{Optimizing the inexact proximal point problem}\label{sec:optz-inxt-pp-problem}
        ...
        \begin{fact}[French Rockafellar Duality {\cite[Proposition 15.22]{bauschke_convex_2017}}]\;\label{fact:fn-rck-duality}\\
...
\section{The inexact accelerated proximal gradient with controlled errors}
    ...
    \begin{proposition}[generic convergence rate of the gradient mapping]\;\label{prop:apg-cnvg-gm-generic}\\
        \todoinline{Cette proposition ira démontrer que le séquence de ``Inexact Gradient Mapping" converge. }
    \end{proposition}
    \begin{proof}
...
\section{Linear convergence for the proximal problem in the inner loop}
    ...
    \begin{definition}[the exact proximal gradient and gradient mapping]\;\label{def:pg-and-gm}\\
        ...
        We define the exact proximal gradient mapping and, gradient mapping: 
        \begin{enumerate}[nosep]
        ...
        \end{enumerate}
    \end{definition}
    \begin{definition}[error bound]\;\label{def:err-bnd}\\
        Let $F = f + g$ where $f:\RR^n \rightarrow \RR$ is a differentiable function and, $g: \RR^n \rightarrow \overline \RR$ is closed convex and proper. 
        Assume the set of minimizers $S = \argmin_{x} F \neq \emptyset$. 
        Let $\gamma> 0$, and let proximal gradient mapping $T_\tau$, gradient mapping $\mathcal G_\tau$ of $F = f + g$ as given by Definition \ref{def:pg-and-gm}. 
        Then their sum $F = f + g$ satisfies the error bound condition if there exists $\gamma > 0$ such that 
    ...
    \subsection{Concrete examples where we can have linear convergence}
        ...
        The goal of this section is to show that specific type of nonsmooth function $\omega$ that common appear in applications satisfies the error bound condition for the proximal problem, and hence the results from the previous section are applicable. 
        \begin{definition}[quasi-strongly convex function {\cite[Definition 1]{necoara_linear_2019}}]
            Let $f:\RR^n \rightarrow \RR$ be differentiable. 
            ...
            It is quasi strongly-convex on the set $X \subseteq \RR^n$ if $\exists \kappa > 0$ such that $\forall x \in X$, let $\bar x = \Pi_{X^+} x$, it satisfies
            \begin{align*}
                f^+ - f(x) - \langle \nabla f(x), \bar x - x\rangle + \frac{\kappa}{2}\Vert x - \bar x\Vert^2 \quad \left(\forall x \in X\right). 
            \end{align*}
        \end{definition}
        \begin{definition}[quadratic growth]\label{def:q-growth}
            Let $F = f + g$ where $f: \RR^n \rightarrow \RR$ be differentiable and, $g: \RR^n \rightarrow \overline \RR$ be closed convex and, proper. 
        ...
        \begin{theorem}[quadratic growth implies error bound]\;\label{thm:qfg-means-eb}\\
            Let $f: \RR^n \rightarrow $
        \end{theorem}
        \begin{proof}
            Denote $x^+ = T_\tau(x)$, $\bar x = \Pi_{S}x$, and $\bar x ^+ = \Pi_{S} x^+$. 
            We use the exact proximal gradient inequality by applying Theorem \ref{thm:inxt-pg-ineq} with $\epsilon = 0, \rho = 0$ and, $z = \bar x^+$ gives: 
            \begin{align*}
            ...
            \end{align*}
            At (1), we used quadratic growth assumption and substitute the inequality of Definition \ref{def:q-growth}, and the Cauchy inequality. 
        ...
        \end{proof}
        \begin{remark}
            This theorem is not exactly a new result. 
            We adapted claim and the proof here so the notations stays consistent and to make it self contained. 

        \end{remark}
        \begin{fact}[Hoffman error bound]
            
        \end{fact}
        \begin{fact}[a type of quasi strongly convex function]
