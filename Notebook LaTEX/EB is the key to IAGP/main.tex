    ...
    \subsection{Optimizing an inexact proximal point problem}\label{sec:optz-inxt-pp-problem}
        ...
        \begin{assumption}[linear composite of convex nonsmooth function]\;\label{ass:for-inxt-prox}\\
            ...
            \begin{enumerate}[nosep]
                ...
                \item $\omega$ is $K_\omega$ Lipschitz continuous, and $\omega^\star$ is $K_{\omega^\star}$ Lipschitz continuous and in addition, $\dom \omega^\star$ is a compact set. 
            \end{enumerate}
        \end{assumption}
        ...
        \begin{proof}
            ...
            \begin{align*}
                \Phi_\lambda(v_j) - \Phi_\lambda(\bar v)
            ...
            \begin{align*}
                ...
                K_\omega\Vert A\Vert \Vert z_j - \bar z\Vert 
                + \frac{1}{2\lambda}\left(
                    2\Vert z_j - y\Vert + \Vert \bar z - z_j\Vert 
                \right)\Vert z_j - \bar z\Vert
                \\
                &= \Vert z_j - \bar z\Vert\left(
                    K_\omega \Vert A\Vert 
                    + \lambda^{-1}\Vert z_j - y\Vert 
                    + \frac{1}{2\lambda}\Vert \bar z - z_j\Vert
                \right)
                \\
...
\section{The inexact accelerated proximal gradient with controlled errors}
    ...
    \begin{proposition}[generic convergence rate of the gradient mapping]\;\label{prop:apg-cnvg-gm-generic}\\
        \todoinline{
            \par 
            Cette proposition ira démontrer que ce séquence de ``Inexact Gradient Mapping" converge. \\
...
\section{Linear convergence for the proximal problem in the inner loop}
    ...
    \subsection{Conditions for linear convergence of the proximal problem}\label{sec:conds-lin-cnvg-pp}
        ...
        \begin{proposition}[sufficient conditions of linear convergence of the inner loop]\;\label{prop:inn-loop-lin-cnvg}\\
            ...
            \begin{enumerate}[nosep]
                ...
                \item\label{prop:inn-loop-lin-cnvg:item3} The duality gap has a linear convergence rate vy the inequality: 
                $$
                    \mathbf G_\lambda(z_j, v_j) \le
                    \left(
                    ...
                    \right). 
                $$
                \item\label{prop:inn-loop-lin-cnvg:item4} If $\mathbf G_\lambda(z_j, v_j) \le \epsilon$, then $z_j \approx_\epsilon \hprox_{\lambda g}(y)$. If we let $\tau = \lambda \Vert AA^\top\Vert$, then a sufficient number of iteration needed to achieve the optimality is given by: 
                \todoinline{
                    N'obliez pas de finir! 
                }
            \end{enumerate}
        \end{proposition}
        \begin{proof}
            ...
            The smooth part of $\Phi$ is the quadratic $h(v) = \frac{1}{2\lambda}\Vert \lambda A^\top v - y\Vert^2 - \frac{1}{2\lambda}\Vert y\Vert^2$, it's locally Lipschitz on the compact the domain of $\dom\Psi_\lambda = \dom \omega^\star$ is compact by Assumption \ref{ass:for-inxt-prox}, hence the smooth part is Lipschitz on $\dom \omega^\star$. 
            ...
            \begin{align*}
                ...
                \\
                &\le \sqrt{2\lambda(\Psi_\lambda(v_j) - \Psi_\lambda(\bar v))}\left(
                    K_\omega \Vert A\Vert 
                    + \lambda^{-1}\Vert z_j - y\Vert 
                    + \frac{1}{2\lambda}\sqrt{2\lambda\left(\Psi_\lambda(v_j) - \Psi_\lambda(\bar v)\right)}
                \right) 
                \\&\quad 
                ...
                \sqrt{2\lambda(\Psi_\lambda(v_j) - \Psi_\lambda(\bar v))}\left(
                    K_\omega \Vert A\Vert 
                    + \lambda^{-1}\Vert z_j - y\Vert 
                    + \frac{\sqrt{2\lambda}}{\lambda}\sqrt{\left(\Psi_\lambda(v_j) - \Psi_\lambda(\bar v)\right)}
                \right) 
                \\
                &\underset{(1)}{\le} 
                \left(
            ...
            \par
            \todoinline{
                N'obliez pas à finir la demonstration d'article (iv). 
            }
        \end{proof}
        \begin{remark}
    ...
    \subsection{Concrete examples where we can have linear convergence}
        The goal of this section is to show that specific type of nonsmooth function $\omega$ that common appear in applications satisfies the error bound condition for the proximal problem, and hence the results from the previous section are applicable and relevant to practical applications. 
        \par
        ...
        \begin{proposition}[1-norm problem has a linear convergence rate]\;\label{prop:1nrm-prox-problem}\\
            ...
            \begin{enumerate}[nosep]
                ...
                $\Psi$ is quasi strongly convex, with $\kappa =\lambda/\theta^2$ where $\theta$ is Hoffman constant of polyhedral set $X^+ = \argmin_{x\in \RR^n}\Psi_\lambda$, and Lipschitz smoothness constant $L_\Psi = \lambda \Vert A^\top\Vert^2$. 
    ...
    \subsection{Inner loop complexity}
        ...
        \begin{assumption}[globally bounded inner loop complexity]\;\label{ass:inn-cmplx}\\
            ...
            \begin{enumerate}[nosep]
                ...
                $$ 
                \item\label{ass:inn-cmplx:item2} There exists a constant $C_0$ such that all the initial guesses given by an algorithm in the outer loop $v_0^{(k)}$ and minimizers $\bar v^{(k)}$ satisfies $C_0 = \sup_{k \in \N \cup \{0\}}\left\Vert v_0^{(k)} - \bar v^{(k)}\right\Vert < \infty$. 
                \item\label{ass:inn-cmplx:item3} There exists the smallest error bound constant $\gamma^{\min}$ such that it lower bound all the constants for the proximal problem, i.e: $\exists \gamma_{\min}:\;0 < \gamma^{\min} \le \inf_{k \in \N \cup \{0\}} \gamma^{(k)}_\lambda$. 
                \item\label{ass:inn-cmplx:item4} $\tilde y_k, \Phi_\lambda^{(k)}, \Psi_\lambda^{(k)}, L^{\lambda, (k)}_{\Phi} ,\gamma_\lambda^{(k)}$ all satisfies error bound conditions (Assumption \ref{ass:lin-cnvg-for-pp}) with $y = \tilde y_k, \Phi_\lambda = \Phi_\lambda^{(k)}, \Psi_\lambda = \Psi_\lambda^{(k)}, L_\Phi^\lambda = L^{\lambda, (k)}_{\Phi}$ and $\gamma_\lambda = \gamma_\lambda^{(k)}$. 
                \item\label{ass:inn-cmplx:item5} The iterates of the inner loop $(z_j^{(k)}, v_j^{(k)})_{k \ge 0}$ are produced by an algorithm that satisfies Definition \ref{def:ista-inner-lp}. 
                \item\label{ass:inn-cmplx:item6}
            \end{enumerate}
        \end{assumption}
        \par
        ...
        \begin{proof}
            ...
            \par
            Before we start, note that $\Phi_\lambda^{(k)}$ is $L_{\Phi}^{\lambda, (k)}$ Lipschitz continuous from Assumption \ref{ass:lin-cnvg-for-pp} because it's assumed in item \ref{ass:inn-cmplx:item4} of Assumption \ref{ass:inn-cmplx}, and the Lipschitz constant only depends on $\lambda^{(k)}$. 
            \par
            To see \ref{prop:inner-lp-cmplx-result2},
            Recall the definition of $\Phi_\lambda^{(k)}$ from \eqref{eqn:primal-pp-k} which implies $\forall k \ge 0$, $L_{\Phi}^{\lambda, (k)}$ is the largest when $\lambda = \lambda^{\max}$. 
            We had $\lambda^{\max}$ is finite, so it has $L^{\max}_\Phi = \sup_{k \in \N \cup \{0\}}L_{\Phi}^{\lambda, (k)} < \infty$ too. 
            \par
            To see \ref{prop:inn-looplin-cnvg-item3}, we use \ref{prop:inn-looplin-cnvg-item2} and, $C_0 = \sup_{k \in \{0\}\cup \N} \left\Vert v_0^{(k)} - \bar v^{(k)} \right\Vert < \infty$ from Assumption \ref{ass:inn-cmplx}, then it's sufficient to have: $C_1 = L_{\Phi}^{\max} C_0$ by $L^{(k)}_\Phi$ Lipschitz continuity of $\Phi_\lambda^{(k)}$ as assumed in Assumption \ref{ass:lin-cnvg-for-pp}. 
            \par
            ...
            Now, substitute $j = N_\epsilon$ then it's prominent that $\mathbf G_\lambda^{(k)}\left(z_j^{(k)}, v_j^{(k)}\right) \le \epsilon$ by using a logarithmic formula. 
        \end{proof}
