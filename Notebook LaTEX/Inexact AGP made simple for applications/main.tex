...
\section{Linear convergence for the proximal problem in the inner loop}
    ...
    \subsection{Conditions for linear convergence of the proximal problem}
        ...
        \begin{assumption}[conditions for linear convergence of proximal problem]\;\label{ass:lin-cnvg-for-pp}\\
            This assumption is about $(g, \omega, A, \Phi_\lambda, \Psi_\lambda, L^\lambda_{\Phi} ,\gamma_\lambda)$. 
            Here are the assumptions
            \begin{enumerate}[nosep]
                \item Assume $(g, \omega, A)$ satisfies Assumption \ref{ass:for-inxt-prox}. The primal objective $\Phi_\lambda$ is given by \eqref{eqn:primal-pp}, and dual $\Psi_\lambda$ by \eqref{eqn:dual-pp}. This means if we let $f(v) := \frac{1}{2\lambda}\Vert \lambda A^\top v - y\Vert^2 - \frac{1}{2\lambda}\Vert y\Vert^2, g(v) := \omega^\star(v)$, then $\Psi_\lambda = f + g$. 
                \item Next, assume $\Psi_\lambda = f + g$ satisfies Assumption \ref{ass:pg-eb} by viewing $\Psi_\lambda = F$ and denote $\gamma_\lambda := \gamma$. Note that we can do this because $f$ is obviously Lipschitz smooth. 
                \item Assume the primal objective $\Phi_\lambda$ is $L_\Phi^\lambda$ Lipschitz continuous. 
            \end{enumerate}
        \end{assumption}
        \par
        ...
        \begin{proposition}[sufficient conditions of linear convergence of the inner loop]\;\label{prop:inn-loop-lin-cnvg}\\
            Let the parameters $(g, \omega, A, \Phi_\lambda, \Psi_\lambda, L^\lambda_{\Phi} ,\gamma_\lambda)$ of a proximal problem satisfy Assumption \ref{ass:lin-cnvg-for-pp}. 
            Let $\tau \ge L_\Psi^\lambda$, and let $v_0 \in \dom \Psi_\lambda$ be an initial condition for an algorithm that satisfies Definition \ref{def:ista} with $F(v) = f(v) + g(v)$ and suppose that it generates iterates $(v_n)_{n \ge 0}$. 
            Let $\bar v$ be a minimizer of $\Psi_\lambda$, then the followings are true for all $\epsilon \ge 0$: 
            \begin{enumerate}[nosep]
                \item The sequence $\Psi_\lambda(v_n) - \Psi_\lambda(\bar v)$ converges linearly to zero. \label{prop:inn-looplin-cnvg-item1}
                \item If we let $z_n := y - \lambda A^\top v_n$, and $\bar z := y - \lambda A^\top \bar v$, then $\Phi_\lambda(z_n) - \Phi_\lambda(\bar z)$ converges linearly. \label{prop:inn-looplin-cnvg-item2}
        ...
        \end{proposition}
        \begin{proof}
            To see \ref{prop:inn-looplin-cnvg-item1}, the sequence $\Psi_\lambda(v_n) - \Psi_\lambda(\bar v)$ has linear convergence by Theorem \ref{thm:lin-cnvg-ista-eb} because we assumed that $\Psi_\lambda$ satisfies Assumption \ref{ass:lin-cnvg-for-pp}. 
            ...
            Because the iterates are given by $z_n = y - \lambda A^\top v_n$, and $\bar z = y - \lambda A^\top \bar v$, due assumed Lipschitz continuity of $\Phi_\lambda$, we can use Theorem \ref{fact:minimizing-dual-pp} which gives: 
        ...
        \end{proof}
        \begin{remark}
            In pratice, use the proximal operator of $\omega^\star$ choose a feasible $v_0$. 
        \end{remark}
        ...
        \begin{proposition}[1-norm problem]\label{prop:1nrm-prox-problem}
            ...
            In addition, if $g := \Vert \cdot\Vert_1$, then the function $\Psi_\lambda, \Psi_\lambda$ satisfies Assumption \ref{ass:lin-cnvg-for-pp}. 
        \end{proposition}
        \begin{proof}
