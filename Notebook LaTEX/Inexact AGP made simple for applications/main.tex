...
\section{The inexact accelerated proximal gradient with controlled errors}
    ...
    \subsection{convergence results of the outer loop}
        ...
        \end{assumption}
        \begin{lemma}[the properties of optimal momentum sequence]\;\label{lemma:opt-mmntm-seq}\\
        ...
        \begin{proof}
            ...
            We prove using induction, inductively assume that $\alpha_k \in (0, 1]$. 
            We can solve for $\alpha_k$ using the recursive equality which should always have one of the strictly positive root given by: 
            \begin{align*}
                \alpha_k &=
                ...
                &= 1
            \end{align*}
            At (1) we completed a square inside of the radical, and use the assumption that $\alpha_k > 0$ and, $L_k > 0, L_{k - 1} > 0$ because we had $B_k > 0$: 
            \begin{align*}
    ...
    \subsection{the fastest rate of which the error schedule can shrink}
        ...
        \begin{lemma}[error schedule lower bound]
