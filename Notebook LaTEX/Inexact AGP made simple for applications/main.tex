...
\section{The inexact accelerated proximal gradient with controlled errors}
    ...
    \subsection{convergence results of the outer loop}
        ...
        \end{assumption}
        \begin{lemma}[the consequencse of optimal momentum sequence]\;\label{lemma:opt-mmntm-seq}\\
            Let $(\alpha_k)_{k \ge 0}, (\beta_k)_{k \ge 0}$ be given by Assumption \ref{ass:opt-mmntm-seq}, then $\forall k\ge 1\; \alpha_k \in (0, 1)$ and, in addition the sequence $(\beta_k)_{k \ge 0}$ has
            \begin{align*}
                \left(
        ...
        \begin{proof}
            ...
            \end{split}\end{align}
            Combining \eqref{eqn:opt-mmntm-seq-pitem1}, \eqref{eqn:opt-mmntm-seq-pitem2} it would mean for all $i \ge 1$ it has: 
            \begin{align*}
                L_0 \alpha_0^2 L_i^{-1} &= 
                \beta_i^{-1}\left(
                ...
                \right)^2. 
            \end{align*}
            Since $\beta_i$ is monotone decreasing, it has $\beta_i^{-1/2}\beta_{i - 1}^{-1/2} \le 1$, and since $\alpha \in (0, 1)$
            \todoinline{HERE! IT'S NOT FINISHED YET!!!}
        \end{proof}
        \begin{proposition}[$\mathcal O(1/k^2)$ optimal convergence rate of the outer loop]\;\label{prop:opt-cnvg-outr-loop}\\
            ...
            \begin{enumerate}[nosep]
                ...
                % \item the sequence $\rho_k$ must also recursively satisfies $\rho_k \ge \max\left(-B_k + \alpha_{k - 1}^4(\rho_{k - 1} + B_{k - 1}), 0\right)$. 
            \end{enumerate}
        ...
        \begin{proof}
            ...
            Finally, one can solve for $\alpha_k$ by the recursive equality, and it should always have one of the strictly positive root: 
            \begin{align*}
                \alpha_k &=
                ...
                \right)
                \\
                &\underset{(1)}{\le} \frac{\alpha_{k - 1}^2L_{k - 1}}{2L_k}\left(
                    - 1 + 1 + 2 \alpha_{k - 1}^{-2}\frac{L_k}{L_{k - 1}}
                \right)
                \\
                &= 1
            \end{align*}
            At (1) we completed a square inside of the radical: 
            \begin{align*}
                ...
                \\
                &\le (1 + 2L_kL_{k - 1}^{-1}\alpha_{k - 1}^{-2})^2. 
            \end{align*}
            To see that $\alpha_k > 0$, recall that we had the assumption that $B_k > 0$ making $L_k > 0$. 
            If we inductively assume that $\alpha_{k - 1} \in (0, 1]$ then $\alpha_k > 0$ because: 
            \begin{align*}
                \alpha_k &=
                \frac{L_{k - 1}}{2L_k}\left(
                    - \alpha_{k - 1}^2 + \left(
        ...
        \begin{remark}
            ...
            \par
            Pay close attention that $\alpha_k$ had been determined in the above theorem, and $(B_k)_{k \le 0}$ is reserved for potential line search routine, the only parameter left undetermined in Definition \ref{def:inxt-apg} is the over-relaxation sequence $(\rho_k)_{k \ge 0}$, we only need the entire sequence is bounded above. 
            ...
            \par
            The the next subsection, we will show that under the assumption of the above theorem, there exists an error sequence $\epsilon_k$ such that it can never approach $0$ at an arbitrarily fast rate. 
        \end{remark}
