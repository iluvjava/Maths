...
\section{Linear convergence for the proximal problem}
    ...
    The inner loop of the algorithm evaluates $x_k \approx_\epsilon T_{(B + \rho)}(y_k)$ for a given value of $\epsilon, B + \rho$ and $y_k$. 
    In this section, we will derive the sufficient conditions for linear convergence rate of the proximal gradient method can attain linear convergence rate for optimizing the dual formulation $\Psi_\lambda$ \eqref{eqn:dual-pp} of the proximal point problem. 
    \par
    ...
    \subsection{Conditions for linear convergence of the proximal problem}
        ...
        \begin{proposition}[scenario I, 1-norm penalization]\label{prop:inn-loop-lin-cnvg}
