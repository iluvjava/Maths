...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{Practical enhancement from the Nesterov's Monotone Variant}
        ...
        The following assumption will characterize the full scope of discussion in nonconvex objective function. 
    ...
    \section{Restarting with function values for linear convergence}
        ...
        \begin{proof}
            ...
            \begin{enumerate}
                ...
                \item [(f)] We used the monotone property of subroutine $\mathbf M$ in GMAPG again so $F(z_0) \le F(z_{j - 1})$. 
                \item [(g)] We substituted $T_\epsilon = \lceil\ln(2\epsilon^{-2}(F(z_0) - F^+))\rceil$ and, removing $\lceil\cdot\rceil$ to make for the $\le$ inequality. 
            \end{enumerate}
        ...
        \begin{proof}
            ...
            Finally, since the doubling must stop, Lemma \ref{lemma:rgmapg-outer-itr-bnd} applies hence, it terminates at most $j + T_\epsilon$ iteration. 
            \par
            Using it let's assume that it executed for $j = 1, 2, \cdots, J$, and it can be represented as $J = m + nT_\epsilon$ with $0 \le m < T_\epsilon$. 
