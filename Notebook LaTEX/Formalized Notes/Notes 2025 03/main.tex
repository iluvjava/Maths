...
\chapter{Linear Convergence of First Order Method}
    ...
    \section{Necoara's et al.'s Paper}
        ...
        \subsection{Characterizing Q-SCNVX functions with Hoffman's error bound}
            ...
            Necoara re-introduce the bound, the following theorem phrase that upper bound in the exact same way, but with only the Euclidean norm. 
            ...
            The following theorem stated their use of the Error bound condition to capture a class of Q-SCNVX function. 
            \begin{theorem}[Q-SCNVX and Hoffman's error bound]\label{thm:q-scnvx-hffmn-eb}
                ...
                Then $f(x):= g(Ax)$ is Q-SCNVX with $L_f = L_g\Vert A\Vert^2$, $\kappa_f = \sigma_g/\theta(A, C)^2$. 
            \end{theorem}
            \begin{proof}
                $X\neq \emptyset$ determines a non-empty set of minimizers $X^+ := \argmin_{x \in X} f(x)$. 
                ...
                \begin{enumerate}
                    ...
                    \item [(b)] There exists uniquely $(t^*, T^*) \in \RR^{m\times n}$ such that for all $\bar x \in X^+$ it has $Ax^+ = t^*, \nabla f(\bar x) = T^*$. In other word, for all minimizers $\bar x$ in $X^+$, $A\bar x$ and, $\nabla f(\bar x)$ are all the same. It would mean $X^+ = \{\bar x : A\bar x = t^*, Cx^* \le d\}$ and, it's also a polyhedron. 
                ...
                For all $x \in \RR^n$ denote $\bar x = \Pi_{X^+} x$, using strong convexity of $g$, and $f(x) = g(Ax)$: 
                \begin{align*}
                ...
                The set $X^+$ is a convex and, $f$ takes the same value all over it. 
