...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{FISTA made simple}
        ...
        % ======================================================================
        \begin{theorem}[generic GMAPG LS gradient mapping convergence]\;\label{thm:gmapg-generic-gradmap-convergence}\\
            ...
            If in addition, the sequence $(\alpha_k)_{k \ge 0}$ has $\alpha_0 = 1$ and, GMAPG is initialized with $v_0=x_0 = T_{1/L_0, f, g}(x_{-1})$ for any $x_{-1} \in\RR^n$ and there exists $x^+$ which is a minimizer of $F$. 
        ...
        \begin{remark}
            ...
            For all $k \ge 1$ let $\hat L = \max_{i = 0, 1, \ldots, k} L_k$ then 
            \begin{align*}
                \beta_k 
                &\le \left(
                    1 + 
                    \frac{\alpha_0 \sqrt{L_0}}{2}\sum_{i = 1}^{k}\sqrt{\hat L^{-1}}
                \right)^{-2}
                \le \left(
                    1 + \frac{1}{2}\alpha_0 \sqrt{L_0}k\sqrt{\hat L^{-1}}
                \right)^{-2}
                \\
                &= \left(
                    1 + \frac{k\alpha_0\sqrt{L_0 \hat L^{-1}}}{2}
                \right)^{-2} = L^{-1}_0\hat L\left(
                    \sqrt{L_0^{-1}\hat L} + \frac{k\alpha_0}{2}
                \right)^{-2}
                \\
                &\le 
                L^{-1}_0\hat L\left(
