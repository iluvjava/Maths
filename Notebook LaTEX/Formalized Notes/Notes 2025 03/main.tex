...
\chapter{Application, KKT of linear programming}
    This chapter extends ideas by the end of Necoara et al.'s paper \cite{necoara_linear_2019}. 
    \section{Introductions and problem formulations}
...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{Restarting with function values for linear convergence}
        ...
        \begin{algorithm}[H]
            ...
            \STATE{$z_1, p_0, \overline L_1, G^+:= \textbf{GMAPG}(f, g, x_{-1}, L, r_j, \rho, N_{\min}=n_0, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
            \STATE{$n_1:=p_0$.}
            \STATE{$M := M - n_1$.}
            \FOR{$j = 1,2, \ldots, M$}
                \IF{$M \le 0 \textbf{ or } G^+ \le \epsilon$}
                    \STATE{\textbf{break}}                
                \ENDIF
                \STATE{$z_{j + 1}, p_j, \overline L_{j + 1}, G^+ := \textbf{GMAPG}(f, g, z_j, \overline L_j, r, \rho, N_{\min}=n_j, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
                \STATE{$M := M - p_j$.}
        ...
        \begin{theorem}[bounds on the total iterations]
            ...
            Suppose that Algorithm \ref{alg:linear-rgmapg} is applied to it then the total number of iterations is bounded for all $M \in \N$, $\epsilon > 0$ by
            \begin{align*}
                \sum_{i = 0}^{J - 1}p_i &\le 
        ...
        \end{proof}
        \par
        The final results from above theorem provides the convergence rate of iterates and the complexity of restart GMAPG under Assumption \ref{ass:q-growth-ch2}. 
        ...
        \end{theorem}
        \begin{proof}
            Suppose $K = \sum_{i = 0}^{J - 1} p_i$ iteration were performed and, it achieved $G_0^{(J)} \le \epsilon$, 
            Now, let $k > 0$ and let $\epsilon = \sqrt{2}(F(z_0) - F^+)^{1/2}\exp(-k + 1)$ then: 
            \begin{align*}
                \ln \left(
                    \frac{2(F(z_0) - F^+)}{\epsilon^2}
                \right)
            ...
            This gives us 
            \begin{align*}
                G_0^{(J)} &\le \epsilon = \sqrt{2}(F(z_0) - F^+)^{1/2}\exp(-k + 1)
                \\
                &=
            ...
            Using Theorem \ref{thm:qfg-peb-equiv} from previous chapter and take note that Assumption \ref{ass:q-growth-ch2} is Definition \ref{def:necoara-weaker-scnvx}\ref{def:necoara-qfg}. 
