...
\chapter{Application, Linear Feasibility Problems}
    ...
    \section{Reducing LP to linear feasibility problems}
        ...
        \begin{definition}[a conic feasibility problem]
...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{Algorithmic description of \XXAPG{}}
        ...
        \subsection{\XXAPG{} main algorithm}
            \begin{algorithm}[H]
                ...
                \caption{\XXAPG{} with Chambolle's backtracking}\label{alg:xxapg}
                \end{algorithmic}
            \end{algorithm}
            The above Algorithm \ref{alg:xxapg} is an implementation of \XXAPG{} in Definition \ref{alg:xxapg}. 
            ...
            \begin{observation}[\XXAPG{} exit conditions]
                Algorithm \ref{alg:xxapg} exits and returns its results on line \ref{alg:xxapg-exit1}, or at line \ref{alg:xxapg-exit2}. 
    ...
    \section{Restarting with function values for linear convergence}
        ...
        The inner loop algorithm is described in Algorithm \ref{alg:xxapg}, let $k$ denote the inner loop counter and define $m = \lfloor k/2 \rfloor + 1$ the conditions are: 
        ...
        \begin{algorithm}[H]
            ...
            \FOR{$j = 1,2, \ldots, M$}
                \IF{$M \le 0 \textbf{ or } G^{(j-1)} \le \epsilon$}
                    \STATE{\textbf{break}}                
                \ENDIF
                \STATE{$z_{j + 1}, p_j, \overline L_{j + 1}, G^{(j)} := \textbf{\XXAPG{}}(f, g, z_j, \overline L_j, r, \rho, N_{\min}=n_j, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
                \STATE{$M := M - p_j$.}
                ...
                \ENDIF
            \ENDFOR
            \end{algorithmic}\caption{Linear convergence restarted \XXAPG{}}\label{alg:linear-rgmapg}
        \end{algorithm}
        \par
        Algorithm \ref{alg:linear-rgmapg} implements a restarted \XXAPG{} with condition $\textbf{E}_{\chi}^a$ and, it has a fast linear convergence rate. 
        The following observation on it is crucial to take notice for the convergence rate. 
        \begin{observation}\label{obs:rgmapg}
            If the outer loop runs for $j = 1, 2, \ldots, J$ iterations with $M \ge J$ so the algorithm terminated due $G^{(J - 1)} \le \epsilon$. 
            Then, it has 
            \begin{enumerate}
                \item for all $J\ge j\ge 1$ it has $n_{j}\le n_{j + 1}$ hence $(n_j)_{j \ge 0}^J$ is monotone increasing; 
                \item for all $J -1 \ge j \ge 1 $ it has $p_{j-1}\le p_{j}$ so $(p_j)_{j \ge 1}^{J}$ is monotone excluding the last iterates $p_J$. 
            \end{enumerate}
        \end{observation}
        \par
        For all $1 \le j \le J$, $n_j$ is used as the lower bound $N_{\min}$ for the \XXAPG{} inner loop described in Algorithm \ref{alg:xxapg} at iteration $j$ and, $p_j$ is the actual iteration underwent by the inner loop therefore it has $p_j \ge n_{j}$. 
        ...
        When $j = J$ it's not necessarily true that $p_J \ge n_J$ because on the last iteration, it can exit the for loop through conditions $G_k^+ \le \epsilon$ in line 14 of Algorithm \ref{alg:xxapg}, but $n_J \ge p_{J - 1} \ge n_{J - 1}$ still. 
        \par
        We now introduce the following notations for the convergence proof. 
        Since the restarted \XXAPG{} consist of executions of an outer loop in Algorithm \ref{alg:linear-rgmapg} in variable $j$ and, an inner loop in Algorithm \ref{alg:xxapg} in variable $k$, we denote $x_{k}^{(j)}$ for the iterates $x_k$ in the inner loop during the $j$ iteration of the outer loop together. 
        ...
        \begin{proposition}[Lipschitz line search estimates are bounded]\label{prop:bnded-lip-ls}
            ...
            Then, the sequence $(\hat L_k)_{k \ge 1}$ from Theorem \ref{thm:xxapg-specialized-cnvg} is bounded above and, it has
            \begin{align*}
                \widehat L_k &:= \max\left(
                    L_0, \left(
                        \frac{1}{k} \sum_{i = 1}^{k} \sqrt{L_i^{-1}}
        ...
        \begin{proof}
            ...
            \end{align*}
            Observe that line 14 of Algorithm \ref{alg:xxapg} exits as soon as possible if $\mathbf E_\chi^a$ is true and $k > N_{\min} = n_j$ and, $G_k \le \epsilon$ will only cause it to exit earlier therefore it has $p_j \le \max(\bar p, n_j)$. 
            \par
            \textbf{Proof of (ii)}. 
        ...
        \begin{proof}
            ...
            \par
            Our goal now is to show that at iteration $j = s$ of Algorithm \ref{alg:linear-rgmapg} it must have $G_0 \le \epsilon$ at line 5 of Algorithm \ref{alg:xxapg}. 
            \par
            Consider the start of the $s$ th iteration of the outer loop, denote $L_0^{(s)}, G_0^{(s)}$ as the $L_0$ in line 4 in Algorithm \ref{alg:xxapg}, it would give the following inequalities
            {\allowdisplaybreaks
            \begin{align*}
                \frac{1}{2}\left(
                    G_0^{(s)}
            ...
            \begin{enumerate}
                \item [(a)] At the $s$ iteration of the loop in Algorithm \ref{alg:linear-rgmapg}, $z_s$ is passed into Algorithm \ref{alg:xxapg} with $x_{-1} = z_s$. Therefore, at line 3 in Algorithm \ref{alg:xxapg} it calls Algorithm \ref{alg:armijo-ls} with $x_{-1} = z_s, \alpha_0 = 1, L^+$ which means $y^+ = x_{-1} = z_s$ at line 5 and $x^+ = T_{1/L^+}(z_s)$. Coming back to line 3 in Algorithm \ref{alg:xxapg}, it assigns $y_0 = y^+ = z_s, x_0 = x^+ = T_{1/L^+}(z_s)$ and $L_0 = L^+$. Assuming the line search went successful, it will have $D_f(z_s, x_0) \le L^{(s)}_0/2 \Vert z_s - x_0\Vert^2$. 
                \item [(b)] We used definition of gradient mapping in Definition \ref{def:gm-for-ch2}. 
                \item [(c)] By the assumption that the line search in Algorithm \ref{alg:armijo-ls} is successful at $z_s$ back in item (a), here we can use \eqref{eqn:emp:result-item-2} in Theorem \ref{thm:xxapg-gm-cnvg}. 
                \item [(d)] \XXAPG{} is monotone in function value for the use of $\mathbf M$ that is either Algorithm \ref{alg:beck-mono} or \ref{alg:nes-mono}, so it has $F\left(x_{p_s}^{(s)}\right) \le F\left(x_0^{(s)}\right)$. 
        ...
        \begin{theorem}[restarted \XXAPG{} convergence and complexity]\;\label{thm:rgmapg-cnvg-complexity}\\
            Let $F = f + g$ satisfy Assumption \ref{ass:q-growth-ch2}, and apply it to Algorithm \ref{alg:linear-rgmapg}. 
        ...
        \end{theorem}
        \begin{proof}
            Suppose a total of $K := \sum_{i = 0}^{J - 1} p_i$ iteration were performed and, at line 10 of Algorithm \ref{alg:linear-rgmapg} at iteration $j = J$ it achieved $G_0 \le \epsilon$ at line 5 of Algorithm \ref{alg:xxapg}. 
            That will cause the inner \XXAPG{} to return $G_0$ so, at line 10 it has $G^{(J)} < \epsilon$ in Algorithm \ref{alg:linear-rgmapg}. 
            ...
            \begin{align}
                \begin{split}
                    ...
                    \right). 
                \end{split}
                \label{ineq:rgmapg-cnvg-complexity-proof-p1}
            \end{align}
            ...
            \begin{align}
                \begin{split}
                    ...
                    \right\Vert. 
                \end{split}
                \label{ineq:rgmapg-cnvg-complexity-proof-p2}
            \end{align}
            }
            \begin{enumerate}
                ...
                \end{align*}
            \end{enumerate}
            Using results of \eqref{ineq:rgmapg-cnvg-complexity-proof-p1}, \eqref{ineq:rgmapg-cnvg-complexity-proof-p2} it has
            \begin{align*}
