...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
            ...
            \begin{algorithm}[H]
                \begin{algorithmic}[1]
                    ...
                    \FOR{$k := 1, 2,\ldots, N$}
                        ...
                        \ENDIF
                    \ENDFOR
                    \STATE{\textbf{Return: }$x_k, k, \overline L$}
    ...
    \section{Restarting with function values for linear convergence}
        ...
        \begin{algorithm}[H]
            ...
            \STATE{$M := M - n_1$}
            \FOR{$j = 1,2, \ldots, M$}
                \STATE{$z_{j + 1}, p_j, \overline L_{j + 1} := \textbf{GMAPG}(f, g, z_j, \overline L_j, r, \rho, N_{\min}=n_j, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
                \STATE{$M := M - p_j$.}
        ...
        We make the rule that $z_{j + 1, -1} = z_{j, p_j}$ for compatibility of the theorems in the previous sections. 
        For example in line 7 at the $j$ iteration, the inner loop returns $x_{p_j}$ as the last iterate of the inner loop and, it's assigned to $z_{j + 1}$ by the outer loop. 
        In that case we denote $z_{j + 1, -1}$ as the initial iterate pass into the inner loop for the $j + 1$ iteration, and $z_{j, p_j}$ would denote the last iteration before exiting the inner loop for the $j$ th iteration of the outer loop. 
        Therefore, $z_{j + 1, -1} = z_{j, p_j}$ and all iterates generated by the inner loop is the finite sequence $(z_{j, k})_{k = 0}^{p_j}$. 
        \par
        ...
        \begin{lemma}[maximum iteration needed for an optimality gap ratio]\label{lemma:prog-ratio}
            Suppose that $F = f + g$ satisfies Assumption \ref{ass:standard-fista}. 
            Denote $F^+ := \inf_xF(x)$ and, let $X^+$ denotes the set of minimizers. 
        ...
        \begin{lemma}[bounds on inner iteration counts]
            ...
        \end{lemma}
        \begin{proof}
