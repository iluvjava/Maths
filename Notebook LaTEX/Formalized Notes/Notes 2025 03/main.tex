...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{FISTA made simple}
        ...
        \begin{definition}[GMAPG LS]\label{def:generic-mapg-ls}\;\\ 
            Initialize with $x_0, v_0, \alpha_0 = 1$. 
            Initialize any $L_0$ such that
            \begin{align*}
                D_f(T_{L_0^{-1}} (y_0)) &\le \frac{L_0}{2}\Vert T_{L_0^{-1}}(x_0) - y_0\Vert^2. 
            \end{align*}
        ...
        \begin{proof}
            ...
            \end{enumerate}
            Now, using Theorem \ref{thm:cnvx-pg-ineq}, it has for all $N \in \N, k = 1, 2, \cdots, N$: 
            {\allowdisplaybreaks
            \begin{align*}
                0 &\le 
                F(z_k) 
        ...
        \begin{example}[MFISTA with Armijo line search]\;\\\vspace{-1em}
            \begin{algorithm}[H]\label{alg:mfista-ls}
                \begin{algorithmic}[1]
                    \STATE{\textbf{Input: }input things }
                    \STATE{}
                \end{algorithmic}
