...
\chapter{Advanced Enhancement Techniques in Accelerated Proximal Gradient}
    ...
    \section{FISTA made simple}
        ...
        Theorem \ref{thm:gmapg-specialized-convergence} gives a concrete $\mathcal O(1/k^2)$ upper bound for the optimality gap and normed gradient mapping on the last iteration. 
        \par
        ...
        \end{proof}
        \begin{remark}
            The techique of the proof we used here is very similar to Güler \cite[Lemma 2.2]{guler_new_1992}.
    ...
    \section{Restarting with function values for linear convergence}
        ...
        \begin{algorithm}[H]
            ...
            \end{tabular}}
            \STATE{$n_0 := 0$.}
            \STATE{$z_1, p_0 := \textbf{GMAPG}(f, g, x_{-1}, r_j, \rho, N_{\min}=n_0, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
            \STATE{$n_1:=p_0. $}
            \STATE{$M := M - n_1$}
            \FOR{$j = 1,2, \ldots, M$}
                \STATE{$z_{j + 1}, p_j := \textbf{GMAPG}(f, g, z_j, r, \rho, N_{\min}=n_j, N=M, \epsilon, \textbf{L}, \textbf{M}, \textbf{E}_{\chi}^a).$}
                \STATE{$M := M - p_j$}
