...
\section{Nesterov's Accelerated Gradient}
    \subsection{In preparations}
        ...
        \end{remark}
        \begin{definition}[semi quasi strong convexity Jensen inequality \textcolor{red}{NEW}]\label{def:sq-scnvx}
            Let $m, n \in \N$. 
            A function $f:\RR^n \rightarrow \overline \RR$ is semi quasi strongly convex with linear mapping $A:\RR^m \rightarrow \RR^n$ if for all $x \in \RR^n, y \in \RR^n$ and $\lambda \in [0, 1]$ it satisfies:
            \begin{align*}
                F(\lambda x + (1 - \lambda)y) \le \lambda F(x) + (1 - \lambda)F(y) -\frac{\lambda(1 - \lambda)}{2} \Vert Ay - Ax\Vert^2. 
            \end{align*}
        \end{definition}
        \begin{remark}
            Choose $A = \sqrt{\mu} I$, where $I$ is the identity matrix then it is equivalent to $\mu$ strongly convex. 
            This is an example of uniform convexity because there exists some $\sigma \ge 0$ such that it fits in Bauschke \cite[Definition 10.7]{bauschke_convex_2017} with $\phi(z) = \sigma^2z^2$. 
            \\
            It is called ``Semi'' because of the semi norm induced by matrix $A$, followed by ``Quasi strongly convex'' because the quasi norm $\Vert Ax\Vert^2$ is the distance of an oblique projection to the kernel of $A$ which is a quasi strongly convex function. 
        \end{remark}
        The above definition is the special case of relatively strong convexity wrt to the Bregman divergence $D_h(x, y) = \Vert Ax - Ay\Vert^2$. 
        \begin{theorem}[an equivalent definition for SQ-SCNVX functions]\label{thm:sq-scnvx-equiv}
            A function $F$ is semi quasi strongly convex with $A$ if and only if the function $x \mapsto F(x) - \frac{1}{2}\Vert A x\Vert^2$ is convex. 
        \end{theorem}
        \begin{proof}
            ...
            So, the function $F(x) - \frac{1}{2}\Vert Ax\Vert^2$ is convex. 
        \end{proof}
        The following theorem classifies a class of semi quasi strongly convex function. 
        \begin{theorem}[Affine composite differentiable SQ-SCNVX functions]\label{thm:smooth-aff-sq-scnvs-fxn}
        ...
        \begin{definition}[semi relative smoothness]\label{def:semi-rel-smooth}
