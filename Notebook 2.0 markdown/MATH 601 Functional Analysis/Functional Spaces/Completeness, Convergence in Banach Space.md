...
The isometry is about the vector space. An example where we can use to complete the metric space $l^\infty$ is [Examples of Incomplete, Complete Metrics Space](Examples%20of%20Incomplete,%20Complete%20Metrics%20Space.md), example 2. All the possible limit of sequences in the metric space will complete the space giving it closure. In the same section, one can also complete the space of polynomials over a closed interval to any continuous function to form a complete metric space (also a vector space) to form a Banach space. 
...
Recall that a metric space is complete when every Cauchy sequences converges to a point that is in the space. To show that we need that the limit of the sequence to be bounded as well, and it exists as a real number too. See [Examples of Incomplete, Complete Metrics Space](Examples%20of%20Incomplete,%20Complete%20Metrics%20Space.md), Example 3
