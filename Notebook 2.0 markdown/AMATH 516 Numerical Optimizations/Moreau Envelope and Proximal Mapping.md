[Multivalued Functions, Set-valued Mappings](Non-Smooth%20Calculus/Multivalued%20Functions,%20Set-valued%20Mappings.md)
...
Dimitry's Comments: The prox operator is a generalization of the set distance operations on sets. see [Set Projections and Dist](Background/Set%20Projections%20and%20Dist.md) for more. in fact their share similarities when the function $f$ is convex, closed and proper. More specifically, prox with a parameter of $1$ gives Lipschitz continuous function with a Lipschitz constant of $L=1$. 
...
To prove we make use of [Strong Convexity, Equivalences and Implications](Strong%20Convexity,%20Equivalences%20and%20Implications.md), and the fact that $x^+$ is the minimum solution of $\text{prox}_f(x)$ and $y^+$ is the minimum of $\text{prox}_f(y)$. 
...
**Proof Lemma**
To show it, we make use of the [Fenchel Identity](Duality/Fenchel%20Identity.md): 
$$
\begin{aligned}
