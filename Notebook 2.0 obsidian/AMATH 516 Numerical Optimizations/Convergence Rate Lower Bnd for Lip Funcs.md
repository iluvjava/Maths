...
Let $\mathcal A^k_fx^{(0)}$ denotes the solution of the k-th iterate $x^{(k)}$ generated by an algorithm $\mathcal A \in \text{GA}^{1}$, with initial guess $x^{(0)}$. An algorithm being optimal globally for steps up to $k$ would mean that 
