...
> For any $1\le k \le 1/2(n + 1)$, for all $x^{(0)}\in \mathbb R^n$, There exists a Lipschitz smooth convex function in $\mathbb R^n$ such that for all algorithm from GA1st, and $k$ such that $1 \le k \le 1/2(n - 1)$ we have the lower bound for the optimality gap for the function values and its iterates: 
>
> $$
> \begin{aligned}
...
#### **Corollary 2.11 | Lower Bound for the Objective value of $f_p$ for Iterates Generated by GA1st**
...
Fix any $1\le k \le 1/2(n + 1)$, and initial value $x^{(0)} = \mathbf 0$, The function we propose is $f(x^{(k)}) = f_{2k + 1}(x^{(k)})$. This function is defined via the iterates produced by the algorithm of GA1st class. 
