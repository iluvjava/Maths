...
Setting $\mu = 0$, the relation simplifies to $\tilde \eta_{t + 1} = \eta_t + L^{-1}$. 
This particular recovers Nesterov original method of accelerated gradient, and also FISTA. 

In the case where the equality $\tilde \eta_{t + 1} = \eta_t + L^{-1} + L^{-1} \tilde \eta_{t +1} + \epsilon_{t + 1}$, where $\epsilon_{t + 1}$ describes errors between the two quantities, then the similar triangle format no longer holds. 
Ctarrying over the error term during the analysis should yield the following relations on the iterates

$$
\begin{aligned}
    x_{t + 1}&= 
    z_{t + 1} + (1 + \mu \tilde \eta_{t + 1})^{-1}L\eta_t (z_{t + 1} - z_t) 
    + 
    \epsilon_{t + 1}(1 + \mu\tilde \eta_{t + 1})^{-1} \mathcal G_L(y_t). 
\end{aligned}
$$
...
> We show that geometrically, that the generic form is the same as a momentum form if and only if there is a similar triangle geometry hidden in the iterates generated by the algorithm. 
...
This is a consequence of one step during the proof when we try to match it to the momentum form, we asserted the condition 
