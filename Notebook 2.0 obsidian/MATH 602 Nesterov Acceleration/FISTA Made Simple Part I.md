...
> Let $(\alpha_k)_{k \ge 0}$ be a sequence in $\R$ such that $\alpha_k\in (0, 1]$ for all $k \ge 0$. 
> Let $(x_k, v_k, y_k)_{k \ge0}$ be the sequence generated by Accelerated Proximal Gradient algorithm. 
> Then, for all $x^+ \in \R^n$, $k \ge 1$: 
> $$
> {\small\begin{aligned}
...
>        \prod_{i =1}^n
>        \max\left(
>            \frac{\alpha_k^2}{\alpha_{k - 1}^2}, 1 - \alpha_k
>        \right)
>    \right)
>    \left(
>        F(x_{k - 1}) - F(x^+)
...
\begin{aligned}
    ...
    &= \left(
        (1 - \alpha_k)(F(x_{k - 1}) - F(x^+))
        + 
        \frac{\alpha_k^2}{\alpha_{k - 1}^2}\frac{L\alpha_{k - 1}^2}{2} \Vert x^+ - v_{k - 1}\Vert^2 
    \right) - (\sim)
    \\
    &= 
    \max\left(
...
Unroll the recurrence for all $k = 1, 2, \ldots, n$ it yields
...
\end{aligned}
$$
It's uncertain how to generalize the above inequality when $g \neq 0$. 
