...
In our case, it's going to be $q = (\mu\sigma(A)^2)/B_k$ where $B_k$ is the inverse of the stepsize from the line search routine. 
...
> Then it satisfies for all $k \ge 1$, the following equalities: 
