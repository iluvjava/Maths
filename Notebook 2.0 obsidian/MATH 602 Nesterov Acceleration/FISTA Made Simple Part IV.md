...
> Suppose that $(F, f, g, h, A, b, \mu, L)$ satisfies Assumption 4. 
> Let the sequence $(y_k, x_k, v_k)_{k \ge 1}$ be generated by RAPG, by applying $F$ described previously, with $q_k = (\mu\sigma(A)^2)/L_k$. 
...
> \end{aligned}
> $$
> Where $\mathcal R_k = \frac{q_kL_k}{2}\Vert \Pi(z_k - y_k)\Vert^2 + \frac{q_kL_k\alpha_k(1 - \alpha_k)}{2}\Vert \Pi(\bar x - x_{k - 1})\Vert^2$. 
