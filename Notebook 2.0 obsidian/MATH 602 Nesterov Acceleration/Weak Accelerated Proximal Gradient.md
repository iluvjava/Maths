...
#### **Algorithm | Relaxed weak accelerated proximal gradient**
...
> Assume $\mu < L$. 
> Suppose that $(y_i, x_i, v_i)$ are generated by the WAPG algorithm for all $0 \le i \le k$ using sequence $(\alpha_k)_{k \ge0}, (\rho_k)_{k \ge0}$ such that for any integer $k\ge 0$, $\alpha_k \in (0, 1)$ and $\rho_k \in [0, 1]$. 
...
Finally, to clear the remianing doubts... 
...
> Given any sequence $(\alpha_i)_{i \ge k}$ such that $\alpha_i \in (0, 1)$ for all $0 \le i \le k$ and $0 < L\alpha_k^2\le (1 - \alpha_k)L\alpha_{k - 1}^2 + \alpha_k \mu$.  
...
> $L\alpha_k^2 = (1 - \alpha_k)L\rho_{k - 1}\alpha_{k - 1}^2 + \alpha_k \mu$ where $\rho_{k} \in [0, 1]\; \forall k \ge0$. 
...
\begin{aligned}
    ...
    &= 
    \frac{\alpha_{k + 1}\alpha_k(1 - \alpha_k)}{\alpha_{k + 1}\alpha_k^2 + \alpha_{k + 1}^2}
    \\
    &= 
    \frac{\alpha_{k + 1}\alpha_k(1 - \alpha_k)}{\alpha_{k + 1}\alpha_k^2 + (1 - \alpha_{k + 1})\alpha_k^2}
    \\
    &= \frac{\alpha_{k + 1}\alpha_k(1 - \alpha_k)}{\alpha_k^2}
    \\
